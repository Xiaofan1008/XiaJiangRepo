function runCSD(LFP,dt,nChn,DEAD)
%% Runs a spline CSD
% variables
...
diam = 1.25e-5; % diameter of electrodes in [m]
el_pos = (-1.45:0.05:0.10)*1e-3; % Vector of positions of each electrode
el_pos(DEAD) = [];
...
npoints = 1000; % number of points to plot in the vertical direction
le = length(zs);
...
c.Label.String = 'm.uA/mm^3';
set(gca,'FontSize',10)
[~,ntime] = size(CSD_cs);
ylim([-50 npoints+50]);
xlim([0 ntime]);
time = (dt:dt:dt*ntime) -1;
chan = (1:npoints/nChn:npoints);
xlabel('Time (ms)')
ylabel('Channels (#)')
set(gca,'XTick',time(1:100:ntime));
set(gca,'YTick',chan);
set(gca,'YTickLabels',1:nChn);
chan(DEAD) = [];
for c = 1:m1
    plot(LFP(c,:) + chan(c),'Color','k');
end
dirName = pwd;
titleName = split(dirName,'\');
titleName = titleName{end};
