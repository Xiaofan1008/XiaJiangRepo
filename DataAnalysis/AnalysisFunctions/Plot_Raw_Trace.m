%% Raw Trace Viewer (sequence-sensitive sets, separate figs for set×amp, subplots for PTDs)
clear all
addpath(genpath('/Volumes/MACData/Data/Data_Xia/AnalysisFunctions/Simple_Analysis/MASSIVE'));

%% ====================== USER SETTINGS ======================
data_folder     = '/Volumes/MACData/Data/Data_Xia/DX013/Xia_Exp1_Single1_251128_113846';

channels_to_plot = [10];              % channels to plot (Depth_s index)
amps_to_plot     = [6];              % amplitudes to include (µA)
ptd_to_plot      = [0];      % PTDs (ms), [] means all
sets_to_plot     = [];                  % stimulation sets, [] means all

nTrials_to_plot  = 10;                  % how many trials to draw per condition
plot_window_ms   = [-5 35];             % window around trigger
...
for ich = 1:length(channels_to_plot)
    ...
    for i_set = 1:length(set_sel)
        ...
        for i_amp = 1:length(amps_sel)
            ...
            figName = sprintf('RawTrace_Ch%d_Set%s_Amp%d', ch_plot, set_label, amp_val);
            ...
            for i_ptd = 1:nPTD
                ...
                for k = 1:numel(tlist)
                    ...
                    plot(time_ms, raw(ch_intan,:), 'LineWidth', 1);
                end

                xline(0,'r--');
                title(sprintf('Inter-Stimulus-Interval %d ms (%d trials)', ptd_val/1000, numel(tlist)));
                xlabel('Time (ms)');
                ylabel('µV');
                ylim([-7000,7000]);
            end

            sgtitle(sprintf('Raw Trace | Ch %d | Set %s | %d µA', ...
                ch_plot, set_label, amp_val));
        end
    end
end
