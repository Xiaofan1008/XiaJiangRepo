...
for i = 1:length(All_Amps)
    ...
    % 3. Plot Grouped Bars
    % b = bar(centers, [N_sim; N_seq]'*100, 'grouped', 'BarWidth', 1.05);

    % 3. Plot Grouped Bars (Manual Offset for Perfect Touching)
    bar_width = hist_bin_width / 2; % Each bar takes up half the bin
    
    % Offset Simultaneous to the LEFT
    b1 = bar(centers - bar_width/2, N_sim*100, 'BarWidth', 0.4, 'FaceColor', 'w', 'EdgeColor', 'none');
    
    % Offset Sequential to the RIGHT
    b2 = bar(centers + bar_width/2, N_seq*100, 'BarWidth', 0.4, 'FaceColor', 'k', 'EdgeColor', 'none');
    
    % Combine them into an array for styling later
    b = [b1, b2];
    
    % --- STYLE SETTINGS ---
    b(1).FaceColor = [0.8 0.8 0.8]; b(1).EdgeColor = 'k'; b(1).LineWidth = 0.1;
    ...
    text((med_sim + med_seq)/2, y_bracket + 1.0, txt, ...
        'FontSize', 14, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');
