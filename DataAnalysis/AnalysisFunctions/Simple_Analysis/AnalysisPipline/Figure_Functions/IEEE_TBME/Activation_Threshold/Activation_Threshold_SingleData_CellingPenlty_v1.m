...
%       3. NEW: Ceiling Penalty (If one condition fails, assign it max Amp).
...
data_file = '/Volumes/MACData/Data/Data_Xia/Analyzed_Results/SpikeCount/DX006/Result_SpikeNormGlobalRef_5uA_Zeroed_5ms_Xia_Exp1_Sim3.mat';
...
for ss = 1:nSets
    for ch = 1:nCh_Total
        ...
        % Note: If it doesn't cross the target, the function returns NaN
        ...
        % [MODIFIED 1] Drop the channel ONLY if BOTH conditions failed to reach 0.5
        if isnan(t_sim) && isnan(t_seq)
            continue;
        end
        
        % [MODIFIED 2] The Ceiling Penalty
        % If one condition reached 0.5 but the other failed, we assign the failing
        % condition a threshold equal to the maximum amplitude we tested.
        ceiling_val = max(Amps); 
        ...
        % [MODIFIED 3] Store the pairs (which now include the penalized data)
...
function thresh = get_interpolated_threshold(amps, curve, target)
    % Ensure arrays are flat 1D vectors to prevent dimension mismatch
    amps = squeeze(amps(:))';
    curve = squeeze(curve(:))';
    ...
    % BUG FIX: If the VERY FIRST valid point is already above the threshold
    if idx_above == 1
        if valid_amps(1) == 0
