...
% [MODIFIED 6] Visual Jitter Settings
% This adds a tiny random shift to the plot coordinates ONLY.
% It does NOT change your stats or the Thresh_Pool arrays.
jitter_amount = 0.15; % +/- 0.15 uA
plot_sim = Thresh_Sim_Pool + (rand(size(Thresh_Sim_Pool)) - 0.5) * 2 * jitter_amount;
plot_seq = Thresh_Seq_Pool + (rand(size(Thresh_Seq_Pool)) - 0.5) * 2 * jitter_amount;

% 1. Plot the Unity Line
plot([0 max_val], [0 max_val], 'k--', 'LineWidth', 1.5, 'HandleVisibility', 'off');

% 2. Scatter the jittered points
% Note: We use the new plot_sim/seq variables here
scatter(plot_sim, plot_seq, dot_size, dot_color, 'filled', ...
    'MarkerFaceAlpha', dot_alpha, 'MarkerEdgeColor', 'none');

% 3. Statistics (Calculated on ORIGINAL non-jittered data)
if p_val < 0.001
