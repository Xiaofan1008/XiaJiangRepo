...
for i = 1:length(All_Amps)
    ...
    fprintf('Amp %.1f uA(N=%d Chns): Sim=%.2f+/-%.2f ms (Median:%.2f) , Seq=%.2f+/-%.2f ms (Median:%.2f)  | Diff=%.2f, p=%.10f\n', ...
        curr_amp, length(seq_data), mean_sim, sem_sim, med_sim, mean_seq, sem_seq, med_seq, median(seq_ch)-median(sim_ch), p_val);
