...
for k = 1:length(fields)
   ...
   % Simultaneous Box (White)
    if ~isempty(d1)
        boxplot(d1, 'Positions', pos1, 'Widths', 0.22, 'Colors', 'k', 'Symbol', '', 'Whisker', box_whisker);
        h = findobj(gca,'Tag','Box'); patch(get(h(1),'XData'), get(h(1),'YData'), 'w', 'FaceAlpha', 1, 'EdgeColor', 'k');
        
        % Manually calculate and draw a bold median line directly on top
        med1 = median(d1);
        plot([pos1-0.11, pos1+0.11], [med1, med1], '-k', 'LineWidth', 1);
    end
    
   % Sequential Box (Grey)
    if ~isempty(d2)
        boxplot(d2, 'Positions', pos2, 'Widths', 0.22, 'Colors', 'k', 'Symbol', '', 'Whisker', box_whisker);
        h = findobj(gca,'Tag','Box'); patch(get(h(1),'XData'), get(h(1),'YData'), [0.8 0.8 0.8], 'FaceAlpha', 1, 'EdgeColor', 'k');
        
        % Manually calculate and draw a bold median line directly on top
        med2 = median(d2);
        plot([pos2-0.11, pos2+0.11], [med2, med2], '-k', 'LineWidth', 1);
    end
...
xlim([0.5, 3.5]); ylim([0, 12]); box off; axis square;
...
if save_figures
    exportgraphics(figure(1), fullfile(save_dir, 'Duration_Binned_BoxPlot.tiff'));
    exportgraphics(figure(2), fullfile(save_dir, 'Duration_Binned_LinePlot.tiff'));
    exportgraphics(figure(3), fullfile(save_dir, 'Duration_Binned_EffectSize.tiff'));
end

fprintf('\n================================================================\n');
fprintf('         STATISTICAL SUMMARY: LINE PLOT (MEANS +/- SEM)         \n');
fprintf('================================================================\n');
...
fprintf('\n================================================================\n');
fprintf('           STATISTICAL SUMMARY: BOX PLOT (MEDIANS)              \n');
fprintf('================================================================\n');
fprintf('%-15s | %-20s | %-20s | %-10s\n', 'Range', 'Sim (Median)', 'Seq (Median)', 'P-Value');
fprintf('----------------------------------------------------------------\n');
for b = 1:length(fields)
    f = fields{b};
    med_sim = median(Bins.(f).Sim);
    med_seq = median(Bins.(f).Seq);
    fprintf('%-15s | %6.2f ms            | %6.2f ms            | %.5f\n', ...
        BinLabels{b}, med_sim, med_seq, BinStats(b).P);
end
fprintf('================================================================\n');