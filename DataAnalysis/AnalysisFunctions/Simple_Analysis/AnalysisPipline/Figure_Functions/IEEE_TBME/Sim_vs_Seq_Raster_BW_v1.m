...
function plot_overlay_figure(fig_num, type_str, r_sim, r_seq, sp_sim, sp_seq, ...
    ...
    % [KEPT] Grey Background Patch for Simultaneous Raster boundary
    ...
    % [MODIFIED 2] Changed X/Y axis font to IEEE Arial 9pt, added bolding for readability
    ...
    % [MODIFIED 3] Uncommented the Raster text labels, updated coordinates, and changed to Arial 9pt
    ...
    % [MODIFIED 4] Enforced global Arial 9pt font across the entire axis
    set(gca, 'FontName', 'Arial', 'FontSize', 9, 'LineWidth', 1.0);
    legend([p1, p2], 'Location', 'northeast', 'Box', 'off', 'FontSize', 9);
    axis square;
    hold off;
end

%% [MODIFIED 5] Save Figure Logic upgraded to 300 DPI TIFF export
if save_figure
