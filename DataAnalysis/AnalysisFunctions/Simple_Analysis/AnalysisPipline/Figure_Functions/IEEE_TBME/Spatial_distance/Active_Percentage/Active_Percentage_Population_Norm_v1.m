...
% --- PANEL 2: Delta Change (Modified to Tuning Curve) ---
nexttile; hold on;
% Changed from bar to errorbar line
errorbar(sum_amps, mDelta(:,1), mDelta(:,2), '-ok', 'LineWidth', 1.5, 'MarkerFaceColor', 'k');
ylabel('\Delta Recruitment (% Total)'); title('Delta Change (Seq - Sim)'); grid on; axis square;
% Add stars relative to the Sequential gain
add_sig_stars(sum_amps, zeros(size(mDelta(:,1))), mDelta(:,1), mDelta(:,2), p_delta);

% --- PANEL 3: Ratio (Modified to Shaded SEM) ---
nexttile; hold on;
...
ylabel('Fold Increase (Seq/Sim)'); title('Ratio (Efficiency)'); grid on; axis square;
ylim([-2 5]); % Adjusted ylim to show the growth better

xlabel(t, 'Amplitude (µA)', 'FontSize', 10, 'FontWeight', 'bold');
