...
% 
%     % Statistics Calculation (Mean & SEM)
...
%     [~, p_delta(end+1)] = ttest(P.Delta, 0); % One-sample t-test vs 0 for delta
% end
% 
% figure('Color','w', 'Units', 'centimeters', 'Position', [5, 5, 26, 8.8], 'Name', 'Normalization_Comparison');
% t = tiledlayout(1,3, 'TileSpacing', 'compact');
% 
% % Panel 1: Standard Normalization (Max-Scaled)
% nexttile; hold on;
% errorbar(sum_amps, mSimStd(:,1), mSimStd(:,2), '--ok', 'MarkerFaceColor','w');
% errorbar(sum_amps, mSeqStd(:,1), mSeqStd(:,2), '-sk', 'MarkerFaceColor','k');
% ylabel('Normalized Potency (Rat Max)'); title('Standard (Max-Scaled)'); grid on; axis square;
% add_sig_stars(sum_amps, mSimStd(:,1), mSeqStd(:,1), mSeqStd(:,2), p_std);
% 
% % Panel 2: Delta Change (Seq - Sim)
% nexttile; hold on;
% y_delta = mDelta(:,1); y_err = mDelta(:,2);
% bar(sum_amps, y_delta, 'FaceColor', [0.8 0.8 0.8]);
% errorbar(sum_amps, y_delta, y_err, 'k.', 'LineWidth', 1);
% ylabel('\Delta Recruitment (% Total)'); title('Delta Change (Seq - Sim)'); grid on; axis square;
% add_sig_stars(sum_amps, zeros(size(y_delta)), y_delta, y_err, p_delta);
% 
% % Panel 3: Ratio (Seq / Sim)
% nexttile; hold on;
% plot([0 10], [1 1], 'r--'); % Baseline line
% errorbar(sum_amps, mRatio(:,1), mRatio(:,2), '-ok', 'MarkerFaceColor','k', 'LineWidth', 1.2);
% ylabel('Fold Increase (Seq/Sim)'); title('Ratio (Baseline Norm)'); grid on; axis square;
% ylim([0.8 2.5]);
% 
% xlabel(t, 'Amplitude (µA)', 'FontSize', 10);
% lg = legend(nexttile(1), {'Sim','Seq'}, 'Location', 'northwest', 'Box', 'off');
...
% t = tiledlayout(1,3, 'TileSpacing', 'compact');
% 
% % --- PANEL 1: Standard Normalization (Line Plot) ---
% nexttile; hold on;
...
% title('Standard (Max-Scaled)', 'FontName', 'Arial'); grid on; axis square;
% add_sig_stars(sum_amps, mSimStd(:,1), mSeqStd(:,1), mSeqStd(:,2), p_std);
% legend('Location', 'northwest', 'Box', 'off');
% 
% % --- PANEL 2: Delta Change (Tuning Curve + Polynomial Fit) ---
% nexttile; hold on;
% % 1. Calculate 3rd-order Polynomial Fit (The Trend)
% coeffs = polyfit(sum_amps, mDelta(:,1), 3);
% xq = linspace(0, 10, 100);
% yq = polyval(coeffs, xq);
% % 2. Plot the smooth trend line behind the points
% plot(xq, yq, '--', 'Color', [0.6 0.6 0.6], 'LineWidth', 2, 'HandleVisibility', 'off'); 
% % 3. Plot raw data tuning curve
% errorbar(sum_amps, mDelta(:,1), mDelta(:,2), '-ok', 'LineWidth', 1.5, 'MarkerFaceColor', 'k');
% ylabel('\Delta Recruitment (% Total)', 'FontName', 'Arial'); 
% title('Delta Change (Seq - Sim)', 'FontName', 'Arial'); grid on; axis square;
% add_sig_stars(sum_amps, zeros(size(mDelta(:,1))), mDelta(:,1), mDelta(:,2), p_delta);
% 
% % --- PANEL 3: Ratio (Shaded SEM + Adjusted Axis) ---
% nexttile; hold on;
% plot([0 10], [1 1], 'r--', 'LineWidth', 1, 'HandleVisibility', 'off'); % Baseline line
% x_vec = sum_amps(:)';
% m_vec = mRatio(:,1)';
% s_vec = mRatio(:,2)';
% % Shaded cloud
...
% title('Ratio (Efficiency)', 'FontName', 'Arial'); grid on; axis square;
% ylim([-2 5]); % Tightened axis to show Ratio growth clearly
% 
...
% --- PANEL 2: Delta Change (Markers + 2nd Order Polynomial) ---
nexttile; hold on;
% 1. Calculate 2nd-order Polynomial Fit (Smoother Trend)
coeffs_delta = polyfit(sum_amps, mDelta(:,1), 2); % Modified from 3 to 2
xq = linspace(0, 10, 100);
yq = polyval(coeffs_delta, xq);
% 2. Plot the smooth trend line
plot(xq, yq, '--', 'Color', [0.4 0.4 0.4], 'LineWidth', 2, 'HandleVisibility', 'off'); 
% 3. Plot raw data markers and error bars (No connecting line)
errorbar(sum_amps, mDelta(:,1), mDelta(:,2), 'ok', 'LineWidth', 1.2, 'MarkerFaceColor', 'k', 'LineStyle', 'none');
ylabel('\Delta Recruitment (% Total)', 'FontName', 'Arial'); 
title('Delta Change (Seq - Sim)', 'FontName', 'Arial'); grid on; axis square;
add_sig_stars(sum_amps, zeros(size(mDelta(:,1))), mDelta(:,1), mDelta(:,2), p_delta);

% --- PANEL 3: Ratio (Shaded SEM + Tight Axis) ---
nexttile; hold on;
...
ylabel('Fold Increase (Seq/Sim)', 'FontName', 'Arial'); 
title('Ratio (Efficiency)', 'FontName', 'Arial'); grid on; axis square;
ylim([-2 5]); % Modified for better visibility of Ratio growth
xlabel(t, 'Amplitude (µA)', 'FontSize', 10, 'FontWeight', 'bold', 'FontName', 'Arial');
