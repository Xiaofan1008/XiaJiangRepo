%% ============================================================
%   SPATIAL ANALYSIS: Single Dataset (Outer Leakage & D90 Containment)
%   - Logic: 
...
for ss = 1:nSets
    ...
    % --- NEW: "BIG POINT" DISTANCE CALCULATION ---
    % 1. Inside target boundary = 0um
    % 2. Outside target boundary = absolute distance to the closest edge
    dist_to_boundary = zeros(length(active_ch), 1);
    is_outer = false(length(active_ch), 1);
    
    for i = 1:length(active_ch)
        if y_coords(i) < y_min
            dist_to_boundary(i) = y_min - y_coords(i); % Leaked below
            is_outer(i) = true;
        elseif y_coords(i) > y_max
            dist_to_boundary(i) = y_coords(i) - y_max; % Leaked above
            is_outer(i) = true;
        else
            dist_to_boundary(i) = 0; % Trapped inside target zone
            is_outer(i) = false;
        end
    end
    ...
    for ai = 1:length(Amps)
        ...
        % (Active Outer / TOTAL Cleaned Active on Shank)
        ...
        % (Uses ALL channels, threshold = 0.90)
...
% Note: Plotting only representative higher amplitudes (e.g., > 2uA) to avoid blank figures
for ss = 1:nSets
    for ai = 1:length(Amps)
        ...
        % Panel 3: D90 Placeholder text (Since D90 is a tuning curve across amps)
        % For the single amp figure, we just print the value. The tuning curve is plotted below.
        nexttile; hold on;
...
%% ================= 6. SAVE RESULTS =================
    ...
    out_name = fullfile(save_dir, ['Result_Spatial_D90_DX016_' exp_id '.mat']);
...
function d_cum = calculate_R_cumulative(dists, resp, threshold)
    % Calculates distance capturing [threshold] percentage of all active channels
...
function metric = calc_binned_prob(dist, resp, valid, edges, mode, total_active_on_shank)
    ...
    for b = 1:length(edges)-1
        ...
        if sum(mask) > 0
            if strcmp(mode, 'prob')
                % Probability = Active in Bin / Available in Bin
                metric(b) = sum(resp(mask)) / sum(mask); 
            elseif strcmp(mode, 'density')
                % Density = Active in Bin / TOTAL Active on entire shank
        ...
        elseif strcmp(mode, 'density')
            metric(b) = 0; % If no channels available in this bin, density is 0
        end
    end
end
