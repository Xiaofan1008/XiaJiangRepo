%% ============================================================
%   SPATIAL ANALYSIS: Single Dataset (RAW RADIUS ARCHIVER)
%   - Logic: 
...
%       4. RADIUS PREVIEW: Calculates a user-defined radius (e.g., D80, D90) for quick view.
%       5. RAW ARCHIVE: Saves exact channel locations so the population script 
%          can dynamically calculate ANY radius later without re-running this script.
% ============================================================
clear; close all;
addpath(genpath('/Volumes/MACData/Data/Data_Xia/AnalysisFunctions'));

%% ================= USER SETTINGS =================
data_folder = '/Volumes/MACData/Data/Data_Xia/DX013/Xia_Exp1_Seq_Sim6';
Seq_PTD_Target = 5; 

% --- NEW: Choose which radius you want to preview in the Command Window ---
% (Note: This only affects the printout. ALL raw data is still saved for the population script)
Target_Radius_Percentile = 0.80; % e.g., 0.90 for D90, 0.80 for D80, 0.75 for D75
...
for ss = 1:nSets
    ...
    for ai = 1:length(Amps)
        ...
        % --- NEW: QUICK PREVIEW CALCULATION ---
        % Calculates chosen radius strictly to print in the Command Window
        SpatialResults.Set(ss).Amp(ai).Preview_Radius_Sim = calculate_R_cumulative(dist_to_boundary, clean_sim, Target_Radius_Percentile);
        SpatialResults.Set(ss).Amp(ai).Preview_Radius_Seq = calculate_R_cumulative(dist_to_boundary, clean_seq, Target_Radius_Percentile);
        
        % --- THE RAW ARCHIVE ---
        % Saves the exact physical arrays so Population script can calculate D90, D80, etc. later
...
fprintf('\n====================================================================\n');
fprintf('     SPATIAL SUMMARY: D%02.0f CONTAINMENT PREVIEW\n', Target_Radius_Percentile * 100);
fprintf('====================================================================\n');
...
for ss = 1:nSets
    for ai = 1:length(Amps)
        ...
        fprintf('Set %02d | %3.1fuA | %5.1fum     | %5.1fum\n', ...
            ss, D.Val, D.Preview_Radius_Sim, D.Preview_Radius_Seq);
    end
end
...
% Saved as 'RawRadiusArchive' to denote it contains the unbinned data
...
%% ================= HELPER FUNCTIONS =================
% Re-added this function so the command window preview works
