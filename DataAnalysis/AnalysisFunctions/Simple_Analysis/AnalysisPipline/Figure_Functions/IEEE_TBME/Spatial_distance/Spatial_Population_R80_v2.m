...
%   - Logic: 
%       1. HD 50um Mapping: 16 bins for 4-shank, 16 bins for 1-shank.
%       2. N-Filter: Only plots amplitudes with N >= 5 datasets.
%       3. Save Choice: High-res TIFF export for publication.
...
save_figs = false; % Set to false if you just want to view the plots
...
for i = 1:length(unique_amps)
    ...
    P = Pooled.(fName);
    if size(P.Sim_Prob_Global, 1) < 5, continue; end 
    
    figure('Units', 'centimeters', 'Position', [2, 2, 10, 8], 'Color', 'w', 'Name', ['Profile_Global_' fName]);
    hold on;
    ...
    xlabel('Distance (\mum)'); ylabel('Probability'); xlim([0 800]); ylim([0 1.1]); grid on;
    title(['N=' num2str(size(P.Sim_Prob_Global,1)) ' | ' num2str(unique_amps(i)) ' \muA']);
end

%% ================= 3. POPULATION DENSITY PLOT =================
fHigh = sprintf('A_%.1f', max(unique_amps(unique_amps <= 10))); fHigh = strrep(fHigh, '.', 'p');
if isfield(Pooled, fHigh)
    P_high = Pooled.(fHigh);
    figure('Units', 'centimeters', 'Position', [5, 5, 12, 10], 'Color', 'w', 'Name', 'Population_Density_Capped');
    hold on;
    mSimNorm = mean(P_high.Sim_Norm_Global, 1, 'omitnan');
    mSeqNorm = mean(P_high.Seq_Norm_Global, 1, 'omitnan');
    x_norm = linspace(0, 800, length(mSimNorm));
    plot(x_norm, mSimNorm, '--ok', 'LineWidth', 1.5, 'MarkerFaceColor', 'w', 'DisplayName', 'Sim');
    plot(x_norm, mSeqNorm, '-sk', 'LineWidth', 1.5, 'MarkerFaceColor', 'k', 'DisplayName', 'Seq');
    xlabel('Distance (\mum)'); ylabel('Fraction of Total Shank Activity');
    title(['Total Density @ ' num2str(max(unique_amps(unique_amps <= 10))) ' \muA']); 
    xlim([0 800]); ylim([0 0.3]); axis square; grid on; legend('Box','off');
end
...
figure('Units', 'centimeters', 'Position', [5, 5, 20, 10], 'Color', 'w', 'Name', 'Population_Trends_Potency_Reach');
t = tiledlayout(1,2);
...
for i = 1:length(sum_amps)
    fprintf('%3.1fuA  | N=%2d | Sim: %4.1f%%, Seq: %4.1f%% | R80 Seq: %4.0fum | p=%.4f\n', ...
        sum_amps(i), n_counts(i), sum_sim_perc(i,1), sum_seq_perc(i,1), sum_seq_r80(i,1), p_perc(i));
end
...
if save_figs
    ...
    for i = 1:length(figHandles)
        ...
        fName = strrep(fName, ' ', '_'); % Clean filename
