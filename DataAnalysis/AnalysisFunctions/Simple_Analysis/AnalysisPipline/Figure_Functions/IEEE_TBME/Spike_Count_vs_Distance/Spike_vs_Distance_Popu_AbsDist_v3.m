...
for aa = 1:length(Unique_Amps)
    ...
    fig_name_bar = sprintf('Spike_vs_Distance_%.1fuA_Bar.png', target_amp);
    fig_name_line = sprintf('Spike_vs_Distance_%.1fuA_Line.png', target_amp);
    
    % ----------------------------------------------------
    % FIGURE 1: BAR PLOT (Grayscale)
    % ----------------------------------------------------
    figure('Units', 'centimeters', 'Position', [2, 2, 9, 9], 'Color', 'w', 'Name', fig_name_bar); 
    hold on;
    ...
    legend( 'Box', 'off', 'FontSize', 10, 'FontName', 'Arial');
    axis square;
    if save_figure
    ...
    % ----------------------------------------------------
    figure('Units', 'centimeters', 'Position', [2, 2, 9, 9], 'Color', 'w', 'Name', fig_name_line); 
    hold on;
    ...
    valid_s = ~isnan(Grand_Sim_Mean) & ~isnan(Grand_Sim_SEM) & ...
              bin_centers >= plot_x_min & bin_centers <= plot_x_max;
    
    x_s   = bin_centers(valid_s);
    y_s   = Grand_Sim_Mean(valid_s);
    sem_s = Grand_Sim_SEM(valid_s);
    
    valid_q = ~isnan(Grand_Seq_Mean) & ~isnan(Grand_Seq_SEM) & ...
              bin_centers >= plot_x_min & bin_centers <= plot_x_max;
    
    x_q   = bin_centers(valid_q);
    y_q   = Grand_Seq_Mean(valid_q);
    sem_q = Grand_Seq_SEM(valid_q);
    ...
    fill([x_s, fliplr(x_s)], [y_s + sem_s, fliplr(y_s - sem_s)], ...
        [0.7 0.7 0.7], 'FaceAlpha', 0.25, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    plot(x_s, y_s, '-o', 'Color', [0.5 0.5 0.5], 'LineWidth', 2, 'MarkerSize', 7, 'MarkerFaceColor', 'w', 'DisplayName', 'Simultaneous');
    ...
    fill([x_q, fliplr(x_q)], [y_q + sem_q, fliplr(y_q - sem_q)], ...
        [0.4 0.4 0.4], 'FaceAlpha', 0.18, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    plot(x_q, y_q, '-s', 'Color', [0 0 0], 'LineWidth', 2, 'MarkerSize', 7, 'MarkerFaceColor', 'w', 'DisplayName', 'Sequential');
    

    for z = 1:length(Stat_Results)
        if isempty(Stat_Results(z).txt) || strcmp(Stat_Results(z).txt, 'n.s.')
            continue;
        end    
        hits = Stat_Results(z).bin_hits;    
        % Plot stars only for bins shown in the figure
        hits_to_plot = hits(bin_centers(hits) >= plot_x_min & ...
                            bin_centers(hits) <= plot_x_max);    
        if isempty(hits_to_plot)
            continue;
        end    
        max_err_sim = max(Grand_Sim_Mean(hits_to_plot) + Grand_Sim_SEM(hits_to_plot));
        max_err_seq = max(Grand_Seq_Mean(hits_to_plot) + Grand_Seq_SEM(hits_to_plot));
        y_top = max(max_err_sim, max_err_seq) + 0.05;
    
        x_star = bin_centers(hits_to_plot);
    
        % Move the left-edge star slightly right to avoid overlap with y-axis
        x_star(x_star == plot_x_min) = x_star(x_star == plot_x_min) + 25;
    
        text(x_star, y_top, Stat_Results(z).txt,'FontSize', 10,'HorizontalAlignment', 'center',  'FontWeight', 'bold', ...
            'FontName', 'Arial');
    end
