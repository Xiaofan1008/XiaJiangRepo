...
%% ================= 1. USER SETTINGS =================
% --- A. File Paths ---
% List the SymmetryProcessed/SpikeCount_vs_Distance .mat files here
file_paths = {
...
for i = 1:length(file_paths)
    ...
    % Only load the 'Results.Amps' variable to save memory and time
    tmp = load(file_paths{i}, 'Results');
    if isfield(tmp, 'Results')
        All_Amps = [All_Amps, tmp.Results.Amps(:)'];
    end
end
% Get unique amplitudes and remove 0 uA (since 0 uA has no stimulation)
...
for aa = 1:length(Unique_Amps)
    ...
    for i = 1:length(file_paths)
        ...
        for ss = 1:length(R.SpatialSets)
            ...
            % 4. Normalize and Store (SELF-NORMALIZED)
    ...
    % Note: You can reduce 'MarkerFaceAlpha' below 0.4 if it is still too cluttered visually.
    jitter = 12; 
    for c = 1:num_bins
...
fprintf('\n>>> All Amplitudes Processed Successfully! <<<\n');