...
%   - Output 3: Required amplitude at selected matched durations
%   - Output 4: Delta required amplitude at selected matched durations
%   - NEW: Constant mean line on delta required amplitude plot
...
fig_delta_const = figure('Color','w', 'Units', 'centimeters', 'Position', [20, 5, 8.8, 8.8], 'Name', 'SelectedMatchedDuration_DeltaAmplitude'); hold on;
...
%% ================= 8. CONSTANT MEAN LINE ON DELTA REQUIRED AMPLITUDE =================
fprintf('\n====================================================================\n');
fprintf('      CONSTANT MEAN LINE: DELTA REQUIRED AMPLITUDE VS DURATION       \n');
fprintf('====================================================================\n');

% IMPORTANT: use only real matched-duration points, NOT artificial 0 point
x_const = Unique_Targets(:);
y_const = Grand_Target_Del_Mean(:);

valid_const = ~isnan(x_const) & ~isnan(y_const);
x_const = x_const(valid_const);
y_const = y_const(valid_const);

if ~isempty(y_const)
    const_mean = mean(y_const, 'omitnan');
    const_sem  = std(y_const, 'omitnan') / sqrt(length(y_const));
    
    fprintf('Mean Delta = %.6f\n', const_mean);
    fprintf('SEM        = %.6f\n', const_sem);
    fprintf('N points    = %d\n', length(y_const));
    
    figure(fig_delta_const); hold on;
    p_const = yline(const_mean, '-', 'Color', [0.3 0.3 0.3], 'LineWidth', 1.2, ...
        'DisplayName', sprintf('Constant mean = %.2f', const_mean));
    
    legend([p6, p_const], {'Sim - Seq', sprintf('Constant mean = %.2f', const_mean)}, ...
        'Location', 'northwest', 'Box', 'off', 'FontSize', 9, 'FontName', 'Arial');
else
    fprintf('No valid points for constant mean line.\n');
