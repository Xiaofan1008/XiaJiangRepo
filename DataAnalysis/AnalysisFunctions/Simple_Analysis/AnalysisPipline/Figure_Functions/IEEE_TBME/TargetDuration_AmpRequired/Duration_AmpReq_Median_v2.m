%% ============================================================
%   GROUP DURATION ANALYSIS: REQUIRED AMPLITUDE AT SELECTED MATCHED DURATIONS
%   - Metric: 3*SD Total Duration
%   - Logic: 1.5x IQR filter -> Strict Pairing -> Median per Set
%   - NEW: Use selected matched-duration targets instead of full inverse curve
...
% --- Selected matched-duration targets (Option B) ---
% target_durations = [2 4 6 8 10];
target_durations = [4:10];
stats_min_n_threshold = 2;
enforce_monotonic = true;

if ~exist(save_dir, 'dir'), mkdir(save_dir); end

%% ================= 1. BUILD PER-FILE CURVES & ESTIMATE SELECTED TARGETS =================
fprintf('Processing %d files (Calculating paired medians and selected matched-duration amplitudes)...\n', length(file_paths));
...
for f = 1:length(file_paths)
    ...
    % Per-file storage at exact amplitude level
    FileCurve = struct();
    FileAmps = [];
    ...
    % Estimate required amplitude only at selected target durations
...
%% ================= 3. PLOT 1: REQUIRED AMPLITUDE AT SELECTED MATCHED DURATIONS =================
fprintf('Generating matched-duration required amplitude plot...\n');
figure('Color','w', 'Units', 'centimeters', 'Position', [5, 5, 8.8, 8.8], 'Name', 'Duration_SelectedTargets'); hold on;
...
% Stats stars
fprintf('\n=== MATCHED DURATION STATS (Paired Signed Rank) ===\n');
...
for k = 1:length(Unique_Targets)
    ...
    if n_pairs < stats_min_n_threshold
        fprintf('Target %.1f ms: Skipped (Low N=%d)\n', target_val, n_pairs);
        continue;
    end
    ...
    if ~isempty(txt)
        ...
        fprintf('Target %.1f ms: MARKED with %s (p=%.5f)\n', target_val, txt, p);
    else
        fprintf('Target %.1f ms: Not Significant (p=%.5f)\n', target_val, p);
    end
end
...
if save_figures
    exportgraphics(gcf, fullfile(save_dir, 'Duration_SelectedTargets_RequiredAmplitude.tiff'), 'ContentType', 'vector', 'Resolution', 300);
end

%% ================= 4. PLOT 2: DELTA REQUIRED AMPLITUDE =================
fprintf('Generating matched-duration delta amplitude plot...\n');
figure('Color','w', 'Units', 'centimeters', 'Position', [10, 5, 8.8, 8.8], 'Name', 'Duration_SelectedTargets_Delta'); hold on;
...
% Stats stars
fprintf('\n=== DELTA REQUIRED AMPLITUDE STATS (Paired Signed Rank) ===\n');
for k = 1:length(Unique_Targets)
    target_val = Unique_Targets(k);
    current_pairs = Pool_Paired(abs(Pool_Paired(:,1)-target_val)<0.001, :);
...
if save_figures
    exportgraphics(gcf, fullfile(save_dir, 'Duration_SelectedTargets_DeltaAmplitude.tiff'), 'ContentType', 'vector', 'Resolution', 300);
end

%% ================= 5. COMMAND WINDOW PRINTOUTS =================
fprintf('\n====================================================================\n');
fprintf('     TABLE 1: REQUIRED AMPLITUDE AT SELECTED MATCHED DURATIONS       \n');
fprintf('====================================================================\n');
...
fprintf('\n====================================================================\n');
fprintf('     TABLE 2: PAIRED SIGNRANK TEST FOR SELECTED MATCHED DURATIONS    \n');
fprintf('====================================================================\n');
