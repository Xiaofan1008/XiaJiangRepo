...
%   - NEW: Paired signed-rank test for matched-duration required amplitude
...
% --- NEW: summary delta containers ---
% each row = [animal_id_numeric, file_index, summary_delta, n_target_points_used]
Summary_ByFile = [];
...
for f = 1:length(file_paths)
    ...
    % --- NEW: collect matched-duration deltas for this file ---
    file_delta_vals = [];
    
    % --- NEW: extract animal ID from file path, e.g. DX010 -> 10 ---
    tok = regexp(file_paths{f}, 'DX(\d+)', 'tokens', 'once');
    if isempty(tok)
        animal_id_num = NaN;
    else
        animal_id_num = str2double(tok{1});
    end
    ...
    for t = 1:length(target_durations)
        ...
        if ~isnan(req_amp_sim) && ~isnan(req_amp_seq)
            ...
            % --- NEW: store file-level matched-duration delta ---
            file_delta_vals = [file_delta_vals; req_amp_sim - req_amp_seq];
        end
    end
    
    % --- NEW: collapse this file to ONE summary delta across matched durations ---
    if ~isempty(file_delta_vals) && ~isnan(animal_id_num)
        this_file_summary_delta = mean(file_delta_vals, 'omitnan');
        Summary_ByFile = [Summary_ByFile; animal_id_num, f, this_file_summary_delta, length(file_delta_vals)];
    end
end
...
%% ================= 5B. NEW OVERALL SUMMARY TEST ACROSS MATCHED DURATIONS =================
fprintf('\nGenerating overall summary delta across matched durations...\n');

% Summary_ByFile columns:
% [animal_id_numeric, file_index, summary_delta, n_target_points_used]

AnimalSummary = [];

overall_animal_p = NaN;
overall_animal_n = 0;
overall_animal_mean = NaN;
overall_animal_sem  = NaN;
overall_animal_median = NaN;

if isempty(Summary_ByFile)
    warning('No valid file-level summary deltas were generated.');
else
    animal_ids = unique(Summary_ByFile(:,1));
    animal_ids = animal_ids(~isnan(animal_ids));
    
    for iA = 1:length(animal_ids)
        this_animal = animal_ids(iA);
        idx_animal = Summary_ByFile(:,1) == this_animal;
        
        deltas_this_animal = Summary_ByFile(idx_animal, 3);
        n_files_this_animal = sum(idx_animal);
        
        animal_delta = mean(deltas_this_animal, 'omitnan');
        AnimalSummary = [AnimalSummary; this_animal, animal_delta, n_files_this_animal];
    end
    
    overall_animal_n = size(AnimalSummary, 1);
    
    if overall_animal_n >= stats_min_n_threshold
        animal_deltas = AnimalSummary(:,2);
        overall_animal_mean   = mean(animal_deltas, 'omitnan');
        overall_animal_sem    = std(animal_deltas, 'omitnan') / sqrt(length(animal_deltas));
        overall_animal_median = median(animal_deltas, 'omitnan');
        
        try
            overall_animal_p = signrank(animal_deltas, 0);
        catch
            overall_animal_p = NaN;
        end
    end
...
for k = 1:length(Unique_Targets)
    target_val = Unique_Targets(k);
    n_pairs = Grand_Target_N(k);
    p = Grand_Target_P(k);
    
    if n_pairs < stats_min_n_threshold
        fprintf('Duration %.1f ms: Skipped (Low N=%d)\n', target_val, n_pairs);
        continue;
    end
    ...
    if ~isempty(txt)
        ...
        fprintf('Duration %.1f ms: Not Significant (p=%s)\n', target_val, format_p(p));
    end
end
...
ylabel('Required Amplitude Difference (Sim - Seq, \muA)', 'FontSize', 9, 'FontName', 'Arial');
xlabel('Matched Duration (ms)', 'FontSize', 9, 'FontName', 'Arial');
...
fprintf('\n====================================================================\n');
fprintf('   TABLE 2: SELECTED MATCHED DURATION REQUIRED AMPLITUDE + STATS    \n');
fprintf('====================================================================\n');
fprintf('%-16s | %-6s | %-16s | %-16s | %-16s | %-12s\n', ...
    'Matched Duration', 'N', 'Sim', 'Seq', 'Delta (Sim-Seq)', 'P-Value');
fprintf('-------------------------------------------------------------------------------------------------\n');

for k = 1:length(Unique_Targets)
    fprintf('%-16.1f | %-6d | %5.2f +/- %4.2f  | %5.2f +/- %4.2f  | %5.2f +/- %4.2f  | %s\n', ...
        Unique_Targets(k), ...
        Grand_Target_N(k), ...
...
fprintf('\n====================================================================\n');
fprintf('   TABLE 3: OVERALL SUMMARY DELTA ACROSS MATCHED DURATIONS (ANIMAL) \n');
fprintf('====================================================================\n');
fprintf('Summary method: mean across target durations\n');
fprintf('Delta definition: Sim required amplitude - Seq required amplitude\n');
fprintf('--------------------------------------------------------------------\n');

if isempty(AnimalSummary)
    fprintf('No valid animal-level summary data available.\n');
else
    fprintf('%-10s | %-18s | %-12s\n', 'Animal', 'Summary Delta', 'N Files');
    fprintf('--------------------------------------------------------------\n');
    
    for iA = 1:size(AnimalSummary,1)
        fprintf('DX%-8d | %-18.4f | %-12d\n', ...
            AnimalSummary(iA,1), AnimalSummary(iA,2), AnimalSummary(iA,3));
    end
    
    fprintf('--------------------------------------------------------------\n');
    fprintf('Animal N              : %d\n', overall_animal_n);
    fprintf('Mean Delta            : %.4f +/- %.4f uA\n', overall_animal_mean, overall_animal_sem);
    fprintf('Median Delta          : %.4f uA\n', overall_animal_median);
    fprintf('Signed-rank p vs 0    : %s\n', format_p(overall_animal_p));
end
