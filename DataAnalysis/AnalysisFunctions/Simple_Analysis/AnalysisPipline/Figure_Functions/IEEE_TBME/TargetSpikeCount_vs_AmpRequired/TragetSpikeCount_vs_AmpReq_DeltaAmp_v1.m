...
%   - Grouping: Bins data by target normalized spike count.
...
fig_name_inverse = 'InverseCurve_RequiredAmplitude_vs_TargetSpikeCount_v1.tiff';
fig_name_delta   = 'InverseCurve_DeltaAmplitude_vs_TargetSpikeCount_v1.tiff';
...
for k = 1:length(Unique_Targets)
    ...
    if ~isempty(txt)
        ...
        text(target_val, y_top + 0.2, txt, 'FontSize', 10, 'HorizontalAlignment', 'center', ...
...
ylim([0, max([Grand_Sim_Mean + Grand_Sim_SEM, Grand_Seq_Mean + Grand_Seq_SEM]) + 1]);
