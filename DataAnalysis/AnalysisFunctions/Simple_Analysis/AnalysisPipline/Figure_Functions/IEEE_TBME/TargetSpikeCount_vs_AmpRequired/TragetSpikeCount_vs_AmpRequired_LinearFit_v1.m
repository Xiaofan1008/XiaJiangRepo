...
%   - Logic: Linear Interpolation per dataset to find "Cost" of spikes.
%   - Style: IEEE TBME Publication Style (Grayscale)
%   - Statistics: Paired Wilcoxon Signed Rank Test at each Target Level
% ============================================================
clear;
...
% Define the target spike counts (Your new X-Axis)
% We stop at 1.6 because not all rats reach 2.0 (prevents NaN errors)
Target_Spikes = 0 : 0.2 : 1.6; 

% --- STATISTICAL FILTERS ---
stats_min_n_threshold = 6; 
bonferroni_n = length(Target_Spikes); 
...
%% ================= 3. LINEAR INTERPOLATION ENGINE (The "Cost" Math) =================
...
for d = 1:length(Dataset_Amps)
    % Force all vectors to be row vectors using (:)' to prevent horzcat errors
    curr_amps = Dataset_Amps{d}(:).';
    curr_sim  = Dataset_SimVals{d}(:).';
    curr_seq  = Dataset_SeqVals{d}(:).';
    
    % Now horizontal concatenation [0, vector] will work perfectly
    curr_amps_0 = [0, curr_amps];
    curr_sim_0  = [0, curr_sim];
    curr_seq_0  = [0, curr_seq];
    
    for t = 1:length(Target_Spikes)
        target = Target_Spikes(t);
        
        % INTERPOLATE: Find Amplitude (X) for a given SpikeCount (Y)
        [u_sim, idx_s] = unique(curr_sim_0);
        if target <= max(u_sim)
            % Cost_Sim(t, d) = interp1(u_sim, curr_amps_0(idx_s), target, 'linear');
            Cost_Sim(t, d) = interp1(u_sim, curr_amps_0(idx_s), target, 'pchip');
        end
        
        [u_seq, idx_q] = unique(curr_seq_0);
        if target <= max(u_seq)
            % Cost_Seq(t, d) = interp1(u_seq, curr_amps_0(idx_q), target, 'linear');
            Cost_Seq(t, d) = interp1(u_seq, curr_amps_0(idx_q), target, 'pchip');
        end
    end
end
...
%% ================= 4. PLOT (Flipped Iso-Response) =================
figure('Units', 'centimeters', 'Position', [2, 2, 8.89, 8.89], 'Color', 'w'); hold on;

% A. Background Scatter (Flipped Axes)
% Since Amps are discrete (Y), we jitter the Spike Counts (X)
jitter_x = 0.05; 
raw_sim = Pool_Flipped_Raw(Pool_Flipped_Raw(:,3)==1, :);
raw_seq = Pool_Flipped_Raw(Pool_Flipped_Raw(:,3)==2, :);

scatter(raw_seq(:,1) + (rand(size(raw_seq,1),1)-0.5)*jitter_x, raw_seq(:,2), 10, [0.7 0.7 0.7], 's', 'filled', 'MarkerFaceAlpha', 0.3, 'HandleVisibility', 'off');
scatter(raw_sim(:,1) + (rand(size(raw_sim,1),1)-0.5)*jitter_x, raw_sim(:,2), 10, [0.8 0.8 0.8], 'o', 'MarkerEdgeAlpha', 0.3, 'HandleVisibility', 'off');


% B. Main Cost Curves
errorbar(Target_Spikes, Grand_Sim_Cost_Mean, Grand_Sim_Cost_SEM, '.', 'Color', 'k', 'LineWidth', 1, 'CapSize', 4, 'HandleVisibility', 'off');
p1 = plot(Target_Spikes, Grand_Sim_Cost_Mean, '--o', 'Color', 'k', 'LineWidth', 1.5, 'MarkerSize', 5, 'MarkerFaceColor', 'w', 'DisplayName', 'Simultaneous');

errorbar(Target_Spikes, Grand_Seq_Cost_Mean, Grand_Seq_Cost_SEM, '.', 'Color', 'k', 'LineWidth', 1, 'CapSize', 4, 'HandleVisibility', 'off');
p2 = plot(Target_Spikes, Grand_Seq_Cost_Mean, '-s', 'Color', 'k', 'LineWidth', 1.5, 'MarkerSize', 5, 'MarkerFaceColor', 'k', 'DisplayName', 'Sequential');

% C. Statistics (Paired Signed Rank at each Target Level)
fprintf('\n=== ISO-RESPONSE STATS (Paired) ===\n');
for t = 1:length(Target_Spikes)
    data_s = Cost_Sim(t, :);
    data_q = Cost_Seq(t, :);
    ...
    if n_pairs >= stats_min_n_threshold
        p = signrank(data_s(valid), data_q(valid));
        
        txt = '';
        if p < (0.001/bonferroni_n), txt = '***';
        elseif p < (0.01/bonferroni_n), txt = '**';
        elseif p < (0.05/bonferroni_n), txt = '*';
        end
        
        if ~isempty(txt)
            % Place star in the gap between the two lines
            y_mid = (Grand_Sim_Cost_Mean(t) + Grand_Seq_Cost_Mean(t)) / 2;
            text(Target_Spikes(t), y_mid, txt, 'FontSize', 10, 'HorizontalAlignment', 'center', 'FontWeight', 'bold', 'FontName', 'Arial');
        end
    end
end
...
axis square;
xlabel('Target Normalized Spike Count (a.u.)', 'FontSize', 9, 'FontName', 'Arial');
