...
%   - Logic: PCHIP Interpolation per dataset (Excluding Origin).
...
% [MODIFIED] Start at 0.2 for the math to prevent the zero-anchor skew
Target_Spikes = 0.1 : 0.2 : 2; 
...
for d = 1:length(Dataset_Amps)
    ...
    for t = 1:length(Target_Spikes)
        ...
        if target <= max(u_sim)
            Cost_Sim(t, d) = interp1(u_sim, curr_amps(idx_s), target, 'linear');
        end
        
        [u_seq, idx_q] = unique(curr_seq);
        if target <= max(u_seq)
            Cost_Seq(t, d) = interp1(u_seq, curr_amps(idx_q), target, 'linear');
        end
    end
end
...
% B. Main Cost Curves 
% [MODIFIED] Add (0,0) ONLY to the lines being drawn, not the math arrays
Plot_X = [0, Target_Spikes];
Plot_Sim_Y = [0, Grand_Sim_Cost_Mean'];
Plot_Seq_Y = [0, Grand_Seq_Cost_Mean'];

% Error bars remain strictly tied to real Target_Spikes
...
for t = 1:length(Target_Spikes)
    ...
    if n_pairs >= stats_min_n_threshold
        ...
        fprintf('Target %.1f | N = %d pairs | p = %.5f %s\n', Target_Spikes(t), n_pairs, p, txt);
