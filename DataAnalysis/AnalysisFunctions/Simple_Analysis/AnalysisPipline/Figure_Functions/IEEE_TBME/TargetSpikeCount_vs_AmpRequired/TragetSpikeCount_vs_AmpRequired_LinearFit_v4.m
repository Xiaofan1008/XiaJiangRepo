...
%   - Logic: PCHIP Interpolation of the Grand Average Curve (Fixes Dropout Bias).
...
% --- PART A: INDIVIDUAL RATS (Kept exclusively for Error Bars & Statistics) ---
...
% Standard Errors are still calculated from the real population variance
...
% --- PART B: THE "GRAND CURVE" METHOD (Calculates true unbiased Means) ---
Unique_Amps = unique(Pool_Flipped_Raw(:,2));
Grand_Spikes_Sim = zeros(size(Unique_Amps));
Grand_Spikes_Seq = zeros(size(Unique_Amps));

% Average all spikes at each amplitude (Matching your forward plot perfectly)
for k = 1:length(Unique_Amps)
    vs = Pool_Flipped_Raw(Pool_Flipped_Raw(:,3)==1 & Pool_Flipped_Raw(:,2)==Unique_Amps(k), 1);
    vq = Pool_Flipped_Raw(Pool_Flipped_Raw(:,3)==2 & Pool_Flipped_Raw(:,2)==Unique_Amps(k), 1);
    Grand_Spikes_Sim(k) = mean(vs, 'omitnan');
    Grand_Spikes_Seq(k) = mean(vq, 'omitnan');
end

% Interpolate the Grand Curve
u_sim_spikes = [0; Grand_Spikes_Sim]; u_sim_amps = [0; Unique_Amps];
u_seq_spikes = [0; Grand_Spikes_Seq]; u_seq_amps = [0; Unique_Amps];

[u_sim_spikes, idx_s] = unique(u_sim_spikes);
[u_seq_spikes, idx_q] = unique(u_seq_spikes);

Grand_Sim_Cost_Mean = nan(length(Target_Spikes), 1);
Grand_Seq_Cost_Mean = nan(length(Target_Spikes), 1);

for t = 1:length(Target_Spikes)
    target = Target_Spikes(t);
    if target <= max(u_sim_spikes)
        Grand_Sim_Cost_Mean(t) = interp1(u_sim_spikes, u_sim_amps(idx_s), target, 'pchip');
    end
    if target <= max(u_seq_spikes)
        Grand_Seq_Cost_Mean(t) = interp1(u_seq_spikes, u_seq_amps(idx_q), target, 'pchip');
    end
end
...
for t = 1:length(Target_Spikes)
    ...
    if n_pairs >= stats_min_n_threshold
        ...
        % Print results directly to the Command Window using the Percentage values
