...
%   - Logic: Power Law Fit (Amp = a * Spikes^b) per dataset.
...
fig_name    = 'IsoResponse_Required_Amplitude_PowerLaw.tiff';
...
%% ================= 3. POWER LAW FITTING ENGINE =================
fprintf('Fitting Power Law (Amp = a*Spikes^b) for %d target spike levels...\n', length(Target_Spikes));
...
for d = 1:length(Dataset_Amps)
    ...
    % FILTER: Power laws require strictly positive values for log transformation
    idx_s = curr_sim > 0.05 & curr_amps > 0;
    idx_q = curr_seq > 0.05 & curr_amps > 0;
    
    % Fit Simultaneous Model
    if sum(idx_s) >= 2
        % Linearize: log(y) = log(a) + b*log(x)
        p_s = polyfit(log(curr_sim(idx_s)), log(curr_amps(idx_s)), 1);
        b_s = p_s(1);        % Exponent
        a_s = exp(p_s(2));   % Scaling factor
        
        for t = 1:length(Target_Spikes)
            target = Target_Spikes(t);
            % Only evaluate if target is within the rat's maximum capacity
            if target <= max(curr_sim)
                Cost_Sim(t, d) = a_s * (target ^ b_s);
            end
        end
    end
    ...
    if sum(idx_q) >= 2
        p_q = polyfit(log(curr_seq(idx_q)), log(curr_amps(idx_q)), 1);
        b_q = p_q(1);
        a_q = exp(p_q(2));
        
        for t = 1:length(Target_Spikes)
            target = Target_Spikes(t);
            if target <= max(curr_seq)
                Cost_Seq(t, d) = a_q * (target ^ b_q);
            end
        end
    end
end
