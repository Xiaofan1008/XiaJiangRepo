...
%   - Logic: Quadratic Fit (2nd Order Poly) per dataset.
...
%% ================= 3. QUADRATIC FITTING ENGINE (The "Cost" Math) =================
fprintf('Fitting 2nd-Order Polynomial for target spike levels...\n');
...
for d = 1:length(Dataset_Amps)
    ...
    % Fit Simultaneous Curve (Requires at least 3 points for a parabola)
    if length(u_sim) >= 3
        p_sim = polyfit(u_sim, curr_amps(idx_s), 2);
        Cost_Sim(:, d) = polyval(p_sim, Target_Spikes);
    ...
    if length(u_seq) >= 3
        p_seq = polyfit(u_seq, curr_amps(idx_q), 2);
        Cost_Seq(:, d) = polyval(p_seq, Target_Spikes);
