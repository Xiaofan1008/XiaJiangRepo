...
for i = 1:length(All_Amps)
    ...
    % --- PERFORM STATISTIC TEST (Rank Sum) ---
    p_val = ranksum(sim_ch, seq_ch);
    fprintf('Amp %.1f uA: Median Diff (Seq-Sim) = %.2f ms, p = %.5f\n', ...
        curr_amp, median(seq_ch)-median(sim_ch), p_val);
    ...
    % --- STYLE SETTINGS ---
    b(1).FaceColor = [0.8 0.8 0.8]; b(1).EdgeColor = 'k'; b(1).LineWidth = 1.0;
...
% [NEW] Perform Global Statistic Test (All Amps Pooled)
