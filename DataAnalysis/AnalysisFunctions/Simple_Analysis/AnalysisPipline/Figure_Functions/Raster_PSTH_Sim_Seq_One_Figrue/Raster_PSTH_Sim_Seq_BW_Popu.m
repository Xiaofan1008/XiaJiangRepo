%% ============================================================
%   Simultaneous vs Sequential: Final Conference Figure (Population)
%   - FIXED: Aggregates multiple channels into one Population Raster/PSTH
% ============================================================
clear;
...
fig_name    = 'Population_Raster_PSTH.tiff';
%% =============== USER SETTINGS ==============================
...
% [MODIFIED] Select multiple channels here for population analysis
% target_channels  = [21]; 
% target_channels  = [9,10,11,12,13,15,19,21,30,31,32]; % Example list
target_channels  = [9,10,11,12,13,15,19,30,31,32]; % Example list
...
%% ============================================================
%                POPULATION AGGREGATION LOOP
% ============================================================
% [MODIFIED] Initialize Master Storage for Population Data
Grand_Spikes_Sim = {}; 
Grand_Spikes_Seq = {};
Grand_Counts_Sim = zeros(1,length(edges)-1);
Grand_Counts_Seq = zeros(1,length(edges)-1);
...
for ch_idx = 1:length(target_channels)
    ...
    % --- Accumulate SIM Data ---
    curr_spikes_sim = cell(numel(trials_sim),1); 
    for i = 1:numel(trials_sim)
        t0 = trig_sim(trials_sim(i))/FS*1000; tt = S_ch_sim(:,1);
        tt = tt(tt>=t0+ras_win(1) & tt<=t0+ras_win(2)) - t0;
        curr_spikes_sim{i} = tt; 
        Grand_Counts_Sim = Grand_Counts_Sim + histcounts(tt,edges);
    end
    % Append current channel's trials to the Grand List
    Grand_Spikes_Sim = [Grand_Spikes_Sim; curr_spikes_sim];
    
    % --- Accumulate SEQ Data ---
    curr_spikes_seq = cell(numel(trials_seq),1); 
    for i = 1:numel(trials_seq)
        t0 = trig_seq(trials_seq(i))/FS*1000; tt = S_ch_seq(:,1);
        tt = tt(tt>=t0+ras_win(1) & tt<=t0+ras_win(2)) - t0;
        curr_spikes_seq{i} = tt; 
        Grand_Counts_Seq = Grand_Counts_Seq + histcounts(tt,edges);
    end
    % Append current channel's trials to the Grand List
    Grand_Spikes_Seq = [Grand_Spikes_Seq; curr_spikes_seq];
end

% [MODIFIED] Print Statistics to Command Window
fprintf('Total Channels Pooled: %d\n', length(target_channels));
fprintf('Total Trials in Raster (Sim): %d\n', numel(Grand_Spikes_Sim));
fprintf('Total Trials in Raster (Seq): %d\n', numel(Grand_Spikes_Seq));

%% ============================================================
%                CALCULATE POPULATION PSTH
% ============================================================
% Calculate mean rate across the ENTIRE population
% Note: Dividing by numel(Grand_Spikes_Sim) averages across (Trials * Channels)
rate_sim_raw = filter(g, 1, Grand_Counts_Sim/(max(1,numel(Grand_Spikes_Sim))*bin_s));
rate_sim_sec = filter(b_sec, a_sec, rate_sim_raw);

rate_seq_raw = filter(g, 1, Grand_Counts_Seq/(max(1,numel(Grand_Spikes_Seq))*bin_s));
...
% ============================================================
% Call the plotting function ONCE with the aggregated data
plot_overlay_figure(1, 'Population Result', rate_sim_sec, rate_seq_sec, ...
    Grand_Spikes_Sim, Grand_Spikes_Seq, ctrs, ras_win, stim_width_ms, ...
    plot_PTD_ms, raster_color, dash_width_ms, -1, plot_amp); 
    % Note: passing -1 as channel ID since it's a population
