...
% Strict first-response validation window. This is recalculated directly
% from the saved unsmoothed PSTHs; it does not use the saved [2,7) early
% summary. Keep its upper boundary at or before the earliest second pulse.
PreSecond_Check_Window_ms = [2 5];
...
if numel(PreSecond_Check_Window_ms) ~= 2 || ...
        any(~isfinite(PreSecond_Check_Window_ms)) || ...
        PreSecond_Check_Window_ms(2) <= PreSecond_Check_Window_ms(1)
    error('CrossPSTH:InvalidPreSecondWindow', ...
        'PreSecond_Check_Window_ms must be [start end] with end > start.');
end
...
for ifile = 1:numel(result_file_paths)
    ...
    for ib = 1:numel(R.branch_averages)
        ...
        for ia = 1:nAmp
            ...
            O.first_component = ...
                row_curve(B.curves.first_component.mean,ia);
            O.shifted_second_component = ...
                row_curve(B.curves.shifted_second_component.mean,ia);

            pre_second_mask = O.time_ms >= PreSecond_Check_Window_ms(1) & ...
                O.time_ms < PreSecond_Check_Window_ms(2);
            if ~any(pre_second_mask)
                error('CrossPSTH:EmptyPreSecondWindow', ...
                    'No PSTH bins occur inside PreSecond_Check_Window_ms.');
            end
            bin_s = double(R.settings.bin_ms)/1000;
            O.pre_second_first_spikes = ...
                sum(O.first_component(pre_second_mask),'omitnan')*bin_s;
            O.pre_second_shifted_second_spikes = ...
                sum(O.shifted_second_component(pre_second_mask),'omitnan')*bin_s;
            O.pre_second_seq_observed_spikes = ...
                sum(O.seq_observed(pre_second_mask),'omitnan')*bin_s;
            O.pre_second_seq_minus_first = ...
                O.pre_second_seq_observed_spikes - ...
                O.pre_second_first_spikes;
            O.pre_second_seq_minus_linear = ...
                O.pre_second_seq_observed_spikes - ...
                O.pre_second_first_spikes - ...
                O.pre_second_shifted_second_spikes;
            ...
            required_curves = [O.sim_observed; O.sim_linear; O.sim_residual; ...
                ...
                O.first_component; O.shifted_second_component];
...
CrossDatasetPSTH.pre_second_check_window_ms = ...
    PreSecond_Check_Window_ms;
...
% Strict pre-second-pulse check, calculated from unsmoothed saved curves.
PreN = zeros(nAmp,1);
PreFirstMean = nan(nAmp,1);
PreFirstSEM = nan(nAmp,1);
PreSeqMean = nan(nAmp,1);
PreSeqSEM = nan(nAmp,1);
PreDiffMean = nan(nAmp,1);
PreDiffSEM = nan(nAmp,1);
PreDiffMedian = nan(nAmp,1);
PrePctPositive = nan(nAmp,1);
PreShiftedSecondMean = nan(nAmp,1);
for ia = 1:nAmp
    idx = CrossDatasetPSTH.branch_indices_by_amplitude{ia};
    first_values = [Observations(idx).pre_second_first_spikes].';
    seq_values = [Observations(idx).pre_second_seq_observed_spikes].';
    diff_values = [Observations(idx).pre_second_seq_minus_first].';
    shifted_values = ...
        [Observations(idx).pre_second_shifted_second_spikes].';
    valid = isfinite(first_values) & isfinite(seq_values) & ...
        isfinite(diff_values) & isfinite(shifted_values);
    first_values = first_values(valid);
    seq_values = seq_values(valid);
    diff_values = diff_values(valid);
    shifted_values = shifted_values(valid);
    PreN(ia) = numel(diff_values);
    [PreFirstMean(ia),PreFirstSEM(ia)] = mean_and_sem(first_values);
    [PreSeqMean(ia),PreSeqSEM(ia)] = mean_and_sem(seq_values);
    [PreDiffMean(ia),PreDiffSEM(ia)] = mean_and_sem(diff_values);
    if ~isempty(diff_values)
        PreDiffMedian(ia) = median(diff_values,'omitnan');
        PrePctPositive(ia) = 100*mean(diff_values>0);
        PreShiftedSecondMean(ia) = mean(shifted_values,'omitnan');
    end
end
PreSecondSummary = table(Amplitudes_uA(:),PreN,PreFirstMean,PreSeqMean, ...
    PreDiffMean,PreDiffSEM,PreDiffMedian,PrePctPositive, ...
    PreShiftedSecondMean, ...
    'VariableNames',{'Amplitude_uA','NBranches','FirstSingle', ...
    'SequentialObserved','SeqMinusFirst','SEMSeqMinusFirst', ...
    'MedianSeqMinusFirst','PctSeqGreaterFirst', ...
    'ShiftedSecondComponent'});
CrossDatasetPSTH.pre_second_summary = PreSecondSummary;
...
disp(WindowSummary);
fprintf('\nStrict pre-second-pulse check [%g,%g) ms (spikes/trial)\n', ...
    PreSecond_Check_Window_ms(1),PreSecond_Check_Window_ms(2));
fprintf('Difference = sequential observed - first single component\n');
disp(PreSecondSummary);
...
% -------- Figure 5: strict pre-second-pulse validation ------------------
figure('Color','w','Position',[100 120 1300 500], ...
    'Name','Pre-second-pulse validation');
tiledlayout(1,2,'TileSpacing','compact','Padding','compact');
sgtitle(sprintf(['Strict pre-second-pulse validation [%g,%g) ms | ' ...
    'unsmoothed counts'],PreSecond_Check_Window_ms(1), ...
    PreSecond_Check_Window_ms(2)),'FontWeight','bold');
...
for ia = 1:nAmp
    ...
    first_values = [Observations(idx).pre_second_first_spikes].';
    seq_values = [Observations(idx).pre_second_seq_observed_spikes].';
    jitter = deterministic_jitter(numel(idx),0.10);
    scatter(ax,Amplitudes_uA(ia)-0.035+jitter,first_values,18, ...
        lighten_color([0.15 0.55 0.20],0.72),'filled', ...
        'HandleVisibility','off');
    scatter(ax,Amplitudes_uA(ia)+0.035+jitter,seq_values,18, ...
        lighten_color(seq_col,0.72),'filled','HandleVisibility','off');
end
errorbar(ax,Amplitudes_uA,PreFirstMean, ...
    sem_for_plot(PreFirstMean,PreFirstSEM),'-o', ...
    'Color',[0.15 0.55 0.20],'MarkerFaceColor',[0.15 0.55 0.20], ...
    'LineWidth',1.8,'CapSize',8,'DisplayName','First single component');
errorbar(ax,Amplitudes_uA,PreSeqMean, ...
    sem_for_plot(PreSeqMean,PreSeqSEM),'-o', ...
    'Color',seq_col,'MarkerFaceColor',seq_col,'LineWidth',1.8, ...
    'CapSize',8,'DisplayName','Sequential observed');
xlabel(ax,'Amplitude (uA)');
ylabel(ax,'Baseline-corrected spikes/trial');
title(ax,'First component versus sequential observation');
legend(ax,'Location','best','Box','off');
box(ax,'off');
...
for ia = 1:nAmp
    ...
    diff_values = [Observations(idx).pre_second_seq_minus_first].';
    jitter = deterministic_jitter(numel(idx),0.10);
    scatter(ax,Amplitudes_uA(ia)+jitter,diff_values,18, ...
        lighten_color(seq_col,0.72),'filled','HandleVisibility','off');
end
errorbar(ax,Amplitudes_uA,PreDiffMean, ...
    sem_for_plot(PreDiffMean,PreDiffSEM),'-o', ...
    'Color',seq_col,'MarkerFaceColor',seq_col,'LineWidth',1.8, ...
    'CapSize',8,'DisplayName','Seq observed - first single');
yline(ax,0,'--','Color',zero_col,'HandleVisibility','off');
xlabel(ax,'Amplitude (uA)');
ylabel(ax,'Difference (spikes/trial)');
title(ax,'Expected value under stable first-pulse response = 0');
legend(ax,'Location','best','Box','off');
box(ax,'off');
...
O = struct( ...
    ...
    'seq_residual',[], ...
    'first_component',[], ...
    'shifted_second_component',[], ...
    'full_difference',[NaN NaN], ...
    'early_difference',[NaN NaN], ...
    'late_difference',[NaN NaN], ...
    'pre_second_first_spikes',NaN, ...
    'pre_second_shifted_second_spikes',NaN, ...
    'pre_second_seq_observed_spikes',NaN, ...
    'pre_second_seq_minus_first',NaN, ...
    'pre_second_seq_minus_linear',NaN, ...
