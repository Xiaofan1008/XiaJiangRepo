...
ax = axes();
hold(ax,'on');
plot_mean_curve(ax,Amplitudes,MeanLinear,SEMLinear,linear_color, ...
    '--o','Linear prediction',Show_Error_Bars);
plot_mean_curve(ax,Amplitudes,MeanSim,SEMSim,sim_color, ...
    '-o','Simultaneous',Show_Error_Bars);
plot_mean_curve(ax,Amplitudes,MeanSeq,SEMSeq,seq_color, ...
    '-o','Sequential',Show_Error_Bars);
xlabel(ax,'Amplitude (uA)');
ylabel(ax,'Baseline-corrected evoked spikes/trial');
title(ax,'Response magnitude [0,20) ms','FontWeight','bold');
...
plot_mean_curve(ax,Amplitudes,MeanSimDifference, ...
    SEMSimDifference,sim_color,'-o', ...
    'Simultaneous - linear',true);

plot_mean_curve(ax,Amplitudes,MeanSeqDifference, ...
    SEMSeqDifference,seq_color,'-o', ...
