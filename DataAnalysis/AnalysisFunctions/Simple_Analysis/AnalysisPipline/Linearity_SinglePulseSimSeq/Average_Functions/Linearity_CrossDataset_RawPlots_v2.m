%% ========================================================================
%  CROSS-DATASET E1: RAW RESPONSE AND NONLINEAR DIFFERENCE PLOTS
%
%  Figure 1
%    Cross-dataset mean +/- SEM of Linear, Simultaneous, and Sequential
%    baseline-corrected evoked spikes/trial.
%
%  Figure 2
%    Cross-dataset median and interquartile range of:
%       Simultaneous - Linear
%       Sequential   - Linear
%
%  Averaging rule
...
Figure_Position = [100 100 800 560];
...
for id = 1:nDatasets
    ...
    for ib = 1:numel(Saved.branch_results)
        ...
        required_columns = {'Amplitude_uA','NUsed','Linear', 'SimMatched','Sequential'};
...
LinearByDataset = nan(nDatasets,nAmp);
...
for id = 1:nDatasets
    ...
    for ia = 1:nAmp
        ...
        for ib = 1:numel(BranchResults)
            ...
            % Use the same complete branch for all three response curves.
        ...
        LinearByDataset(id,ia) = mean(branch_linear);
        SimByDataset(id,ia) = mean(branch_sim);
        SeqByDataset(id,ia) = mean(branch_seq);
        SimDifferenceByDataset(id,ia) = mean(branch_sim-branch_linear);
        SeqDifferenceByDataset(id,ia) = mean(branch_seq-branch_linear);
        NBranchesUsed(id,ia) = numel(branch_linear);
    end
end

ValidComparison = isfinite(LinearByDataset) & isfinite(SimByDataset) & isfinite(SeqByDataset);
...
for id = 1:nDatasets
    for ia = 1:nAmp
        ...
        Linear(row) = LinearByDataset(id,ia);
        ...
        if Valid(row)
            SimSublinear(row) = double(SimMatched(row) < Linear(row));
            SeqSublinear(row) = double(Sequential(row) < Linear(row));
...
DatasetAmplitudeResults = table(RowAnimal,RowDataset,RowLabel, ...
    Amplitude_uA,Valid,IsExcluded,UseInSummary,RowNBranches,Linear, ...
    ...
    'Valid','Excluded','Used','NBranches','Linear','SimMatched', ...
    'Sequential','SimMinusLinear','SeqMinusLinear','SimSublinear', ...
    'SeqSublinear','SeqGreaterSim'});

%% ============================= SUMMARIES ================================

NDatasets = zeros(nAmp,1);
MeanLinear = nan(nAmp,1); SEMLinear = nan(nAmp,1);
...
for ia = 1:nAmp
    ...
    [MeanLinear(ia),SEMLinear(ia),n_linear] = mean_sem(LinearByDataset(use,ia));
    ...
    counts = [n_linear,n_sim,n_seq,n_sim_diff,n_seq_diff];
    ...
    NDatasets(ia) = n_linear;
    if n_linear > 0
        PctSimSublinear(ia) = 100*mean( SimByDataset(use,ia) < LinearByDataset(use,ia));
        PctSeqSublinear(ia) = 100*mean( SeqByDataset(use,ia) < LinearByDataset(use,ia));
        PctSeqGreaterSim(ia) = 100*mean( SeqByDataset(use,ia) > SimByDataset(use,ia));
    end
end

ResponseSummary = table(Amplitudes(:),NDatasets,MeanLinear,SEMLinear, MeanSim,SEMSim,MeanSeq,SEMSeq, ...
    'VariableNames',{'Amplitude_uA','NDatasets','MeanLinear','SEMLinear', 'MeanSim','SEMSim','MeanSeq','SEMSeq'});

DifferenceSummary = table(Amplitudes(:),NDatasets,MeanSimDifference, ...
    ...
    PctSeqGreaterSim, ...
    'VariableNames',{'Amplitude_uA','NDatasets','MeanSimDifference', ...
...
for ia = 1:nAmp
    ...
    for id = find(UseForSummary(:,ia)).'
        ...
        for ib = 1:numel(BranchResults)
            ...
            if T.NUsed(source_row) <= 0 || ~isfinite(T.Linear(source_row)) || ~isfinite(T.SimMatched(source_row)) || ~isfinite(T.Sequential(source_row))
                continue;
            end
...
PlotMeanLinear = MeanLinear;
PlotSEMLinear = SEMLinear;
PlotMeanSim = MeanSim;
PlotSEMSim = SEMSim;
PlotMeanSeq = MeanSeq;
PlotSEMSeq = SEMSeq;
if Add_Zero_Plot_Anchor && ...
    ...
    PlotMeanLinear = [0; PlotMeanLinear];
    PlotSEMLinear = [0; PlotSEMLinear];
...
if Show_Dataset_Scatter
    ...
    plot_dataset_scatter(ax,Amplitudes,LinearByDataset,UseForSummary, ...
        linear_color,-Scatter_Condition_Offset,Scatter_Jitter_Width, ...
        Scatter_Marker_Size,Scatter_Alpha);
    plot_dataset_scatter(ax,Amplitudes,SimByDataset,UseForSummary, ...
        sim_color,0,Scatter_Jitter_Width,Scatter_Marker_Size,Scatter_Alpha);
    plot_dataset_scatter(ax,Amplitudes,SeqByDataset,UseForSummary, ...
        seq_color,Scatter_Condition_Offset,Scatter_Jitter_Width, ...
        Scatter_Marker_Size,Scatter_Alpha);
end

plot_mean_curve(ax,ResponsePlotX,PlotMeanLinear,PlotSEMLinear,linear_color, ...
    '--o','Linear prediction',Show_Error_Bars);
...
title(ax,'Response magnitude [2,20) ms','FontWeight','bold');
