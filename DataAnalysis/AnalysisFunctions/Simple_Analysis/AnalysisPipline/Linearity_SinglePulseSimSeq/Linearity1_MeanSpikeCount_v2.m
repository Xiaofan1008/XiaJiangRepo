...
priority_files = find_named_mat_files(folder,'simseqbadtrials');
if numel(priority_files) > 1
    error('Linearity1:AmbiguousBadTrialFile', ...
        'Multiple SimSeqBadTrials files were found in %s.',folder);
end
if isscalar(priority_files)
    files = priority_files;
else
    files = find_named_mat_files(folder,'badtrials');
    % Do not reconsider the preferred file under its general name.
    general_keep = true(size(files));
    for k = 1:numel(files)
        normalized_name = regexprep(lower(files(k).name),'[^a-z0-9]','');
        general_keep(k) = ~contains(normalized_name,'simseqbadtrials');
    end
    files = files(general_keep);
    if numel(files) > 1
        error('Linearity1:AmbiguousBadTrialFile', ...
            'Multiple BadTrials files were found in %s.',folder);
    end
end
if isempty(files)
...
function files = find_named_mat_files(folder,normalized_token)
files = clean_file_list(dir(fullfile(folder,'*.mat')));
keep = false(size(files));
for k = 1:numel(files)
    normalized_name = regexprep(lower(files(k).name),'[^a-z0-9]','');
    keep(k) = contains(normalized_name,lower(normalized_token));
end
files = files(keep);
end
