...
%    - Saves separate branch/order averages without merging A->B and B->A.
%    - Retains supporting channel-level unsmoothed PSTHs for later checks.
%
...
% Empty creates an automatic filename:
%   PSTHLinearity_<AnimalID>_<DatasetID>.mat
% Alternatively enter a custom filename, with or without the .mat ending.
Custom_Result_File_Name = '';
Overwrite_Existing_Result = true;

% Channel-level unsmoothed PSTHs are recommended because they permit later
% verification and alternative averaging. Trial IDs are useful for audit
% checks but can be disabled if smaller files are preferred.
Save_Channel_Level_Results = true;
Save_Trial_IDs = true;
...
for ip = pair_indices
    ...
    for ib = 1:numel(Branches)
        ...
        P.Sim_residual_raw = Sim_observed_raw-Sim_linear_raw;
        P.Seq_residual_raw = Seq_observed_raw-Seq_linear_raw;
        ...
        P.n_responding_channels_detected = numel(branch_channels);
        P.n_channels_used_by_amplitude = sum(valid_by_amplitude,1);
        P.branch_average = build_branch_average(P);
...
if Save_Results
    PSTHLinearitySaved = build_saved_result(PSTHResults,DetectedPairTable, ...
        Single,Sim,Seq,single_folder,sim_folder,seq_folder, ...
        Electrode_Type,FS,baseline_win_ms,response_win_ms,early_win_ms, ...
        late_win_ms,Sequential_PTD_ms,PTD_Tolerance_ms,display_win_ms, ...
        compute_win_ms,bin_ms,smooth_sigma_ms,Trial_Mode,Random_Seed, ...
        Include_Zero_Amplitude,selected_amplitudes,Channels_To_Analyze, ...
        Save_Channel_Level_Results,Save_Trial_IDs);
    output_path = save_psth_result(PSTHLinearitySaved, ...
        Results_Save_Folder,Custom_Result_File_Name, ...
...
function B = build_branch_average(P)
% Create the unsmoothed branch/order averages that will be the primary
% input to later cross-dataset analysis. The same common valid-channel mask
% is used for every curve at a given amplitude.
B = struct();
B.electrode_A = P.electrode_A;
B.electrode_B = P.electrode_B;
B.pair_key = P.pair_key;
B.branch_code = P.branch_code;
B.branch_label = P.branch_label;
B.sim_order_key = P.sim_order_key;
B.seq_order_key = P.seq_order_key;
B.sim_set_indices = P.sim_set_indices;
B.seq_set_indices = P.seq_set_indices;
B.amplitudes_uA = P.amplitudes_uA;
B.time_ms = P.ctrs_compute_ms;
B.display_mask = P.display_mask;
B.n_responding_channels_detected = P.n_responding_channels_detected;
B.n_channels_used_by_amplitude = P.n_channels_used_by_amplitude;
B.depth_channels_used_by_amplitude = cell(1,numel(P.amplitudes_uA));
for ia = 1:numel(P.amplitudes_uA)
    B.depth_channels_used_by_amplitude{ia} = ...
        P.depth_channels(P.valid_by_amplitude(:,ia));
end

curve_spec = {
    'single_A',                 'A_raw';
    'single_B',                 'B_raw';
    'first_component',          'First_component_raw';
    'shifted_second_component', 'Second_shifted_raw';
    'sim_observed',             'Sim_observed_raw';
    'sim_linear',               'Sim_linear_raw';
    'sim_residual',             'Sim_residual_raw';
    'seq_observed',             'Seq_observed_raw';
    'seq_linear',               'Seq_linear_raw';
    'seq_residual',             'Seq_residual_raw'};

B.curves = struct();
for k = 1:size(curve_spec,1)
    output_name = curve_spec{k,1};
    source_name = curve_spec{k,2};
    [curve_mean,curve_sem] = summarize_curve_array( ...
        P.(source_name),P.valid_by_amplitude);
    B.curves.(output_name) = struct('mean',curve_mean,'sem',curve_sem);
end

[B.full_difference.mean,B.full_difference.sem] = ...
    summarize_metric_array(P.full_difference,P.valid_by_amplitude);
[B.early_difference.mean,B.early_difference.sem] = ...
    summarize_metric_array(P.early_difference,P.valid_by_amplitude);
[B.late_difference.mean,B.late_difference.sem] = ...
    summarize_metric_array(P.late_difference,P.valid_by_amplitude);
B.difference_condition_order = {'Sim','Seq'};
B.summary = P.summary;
end

function [avg,se] = summarize_curve_array(values,valid_mask)
nAmp = size(values,2);
nTime = size(values,3);
avg = nan(nAmp,nTime);
se = nan(nAmp,nTime);
for ia = 1:nAmp
    ...
    n = sum(valid);
    if n == 0, continue; end
    M = reshape(values(valid,ia,:),n,nTime);
    avg(ia,:) = mean(M,1,'omitnan');
    if n > 1
        se(ia,:) = std(M,0,1,'omitnan')/sqrt(n);
    end
end
end

function [avg,se] = summarize_metric_array(values,valid_mask)
nAmp = size(values,2);
nConditions = size(values,3);
avg = nan(nAmp,nConditions);
se = nan(nAmp,nConditions);
for ia = 1:nAmp
    valid = valid_mask(:,ia);
    n = sum(valid);
    if n == 0, continue; end
    M = reshape(values(valid,ia,:),n,nConditions);
    avg(ia,:) = mean(M,1,'omitnan');
    if n > 1
        se(ia,:) = std(M,0,1,'omitnan')/sqrt(n);
    end
...
function Saved = build_saved_result(Results,DetectedPairTable,Single,Sim,Seq, ...
        single_folder,sim_folder,seq_folder,Electrode_Type,FS,baseline_win, ...
        response_win,early_win,late_win,seq_ptd,ptd_tol,display_win, ...
        compute_win,bin_ms,smooth_sigma,trial_mode,random_seed,include_zero, ...
        amplitudes,channel_request,save_channel_level,save_trial_ids)

Saved = struct();
Saved.format_version = '1.0';
Saved.analysis_name = 'Linearity3_PSTH_TemporalLinearity_OrderMatched';
...
Saved.settings = struct();
Saved.settings.electrode_type = Electrode_Type;
Saved.settings.FS = FS;
Saved.settings.baseline_win_ms = baseline_win;
Saved.settings.response_win_ms = response_win;
Saved.settings.early_win_ms = early_win;
Saved.settings.late_win_ms = late_win;
Saved.settings.sequential_ptd_ms = seq_ptd;
Saved.settings.ptd_tolerance_ms = ptd_tol;
Saved.settings.display_win_ms = display_win;
Saved.settings.compute_win_ms = compute_win;
Saved.settings.bin_ms = bin_ms;
Saved.settings.smooth_sigma_ms = smooth_sigma;
Saved.settings.trial_mode = char(string(trial_mode));
Saved.settings.random_seed = random_seed;
Saved.settings.include_zero_amplitude = logical(include_zero);
Saved.settings.amplitudes_uA = amplitudes;
Saved.settings.channels_to_analyze = channel_request;
Saved.settings.channel_population_mode = 'order_matched';
Saved.settings.branch_averaging = ...
    'Equal weight across valid responding channels within each branch';
Saved.settings.cross_branch_averaging_applied = false;
Saved.settings.saved_curves_are_smoothed = false;
Saved.settings.save_channel_level_results = logical(save_channel_level);
Saved.settings.save_trial_ids = logical(save_trial_ids);
...
Saved.branch_results = Results;
if ~save_trial_ids
    for k = 1:numel(Saved.branch_results)
        Saved.branch_results(k).trial_ids_used = {};
    end
end
if ~save_channel_level
    channel_curve_fields = {'A_raw','B_raw','Sim_observed_raw', ...
        'Seq_observed_raw','Sim_linear_raw','Seq_linear_raw', ...
        'Sim_residual_raw','Seq_residual_raw','First_component_raw', ...
        'Second_shifted_raw','full_difference','early_difference', ...
        'late_difference'};
    present = channel_curve_fields(isfield(Saved.branch_results, ...
        channel_curve_fields));
    Saved.branch_results = rmfield(Saved.branch_results,present);
end

% A compact top-level collection makes later loading straightforward while
% the full branch results remain available for verification.
Saved.branch_averages = [Results.branch_average];
Saved.n_saved_branches = numel(Results);
end

function M = source_file_metadata(D)
M = struct('folder',D.folder,'spike_file',D.spike_file, ...
    'spike_variable',D.spike_variable,'trigger_file',D.trigger_file, ...
...
function output_path = save_psth_result(Saved,save_folder,custom_name,overwrite)
save_folder = char(string(save_folder));
if isempty(strtrim(save_folder))
    error('LinearityPSTH:EmptySaveFolder', ...
        'Results_Save_Folder cannot be empty when Save_Results is true.');
end
if ~isfolder(save_folder)
    [ok,message] = mkdir(save_folder);
    if ~ok
        error('LinearityPSTH:CannotCreateSaveFolder', ...
            'Could not create %s: %s',save_folder,message);
    end
end

custom_name = strtrim(char(string(custom_name)));
if isempty(custom_name)
    filename = sprintf('PSTHLinearity_%s_%s.mat', ...
        sanitize_filename(Saved.animal_id), ...
        sanitize_filename(Saved.dataset_id));
else
    [~,base,extension] = fileparts(custom_name);
    base = sanitize_filename(base);
    if isempty(extension)
        extension = '.mat';
    elseif ~strcmpi(extension,'.mat')
        error('LinearityPSTH:InvalidResultExtension', ...
            'Custom_Result_File_Name must have no extension or end in .mat.');
    end
...
if isfile(output_path) && ~overwrite
    error('LinearityPSTH:ResultAlreadyExists', ...
        ['Result already exists:\n%s\nSet Overwrite_Existing_Result=true ' ...
         'to replace it.'],output_path);
end
PSTHLinearitySaved = Saved; %#ok<NASGU>
save(output_path,'PSTHLinearitySaved','-v7.3');
end

function animal_id = detect_animal_id(folder)
animal_id = regexp(upper(char(string(folder))),'DX\d+','match','once');
if isempty(animal_id), animal_id = 'AnimalUnknown'; end
end

function out = sanitize_filename(value)
out = regexprep(char(string(value)),'[^A-Za-z0-9_-]','_');
if isempty(out), out = 'Unknown'; end
end
