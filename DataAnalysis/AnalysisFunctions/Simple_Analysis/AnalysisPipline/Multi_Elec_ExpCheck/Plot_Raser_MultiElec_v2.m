%% Raster + PSTH Viewer for Mixed-Prefix Stimulation Files
% ------------------------------------------------------------
% Purpose:
%   1) Load raw spike file (*.sp.mat)
%   2) Save it as *.sp_xia.mat with variable sp_clipped
...
%   This avoids trial-order misalignment after randomization.
%
% ConditionType:
%   0 = zero-control
...
%% ====================== CHOOSE FOLDER ======================
data_folder = '/Volumes/MACData/Data/Data_Xia/DX027/Xia_FixISI_5ms5';
...
% If true, save .sp.mat as .sp_xia.mat.
save_sp_xia = true;
...
%% ====================== SAVE .sp_xia.mat ======================
% The original .sp.mat contains variable sp.
% Later analysis scripts expect .sp_xia.mat to contain sp_clipped.
% Here we simply rename sp -> sp_clipped. The spike contents are unchanged.
...
%   This keeps metadata aligned with actual trigger/trial order.
%
% StimParams columns:
...
% Usually pulse train period is not the main grouping variable in this prefix design.
                ...
                for pi = 1:n_PULSE
                    ...
                    for aci = 1:numel(Prefixes_selected)
                        ...
                        for ii = 1:numel(ISIs_selected)
                            ...
                            %% ----- Figure -----
                            figName = sprintf('Ch %d | Set %d %s | Prefix %d | ISI %.1f ms | Pulse %d us', ...
            ...
            elseif condType == 2
                ...
                for pi = 1:n_PULSE
                    ...
                    for aci = 1:numel(activeCounts_sim)
                        ...
                        %% ----- Figure -----
                        figName = sprintf('Ch %d | Set %d %s | Simultaneous | Pulse %d us', ...
