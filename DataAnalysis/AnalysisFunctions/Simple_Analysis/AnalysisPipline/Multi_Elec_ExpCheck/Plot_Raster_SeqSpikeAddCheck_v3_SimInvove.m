...
% This ISI filter is only applied to sequential prefix trials.
% Simultaneous trials are included as reference and do not use ISI.
isi_to_plot_ms = [5];

% Stimulation set/order IDs to plot. [] means all detected sets.
sets_to_plot = [];

include_simultaneous_reference = 1; % 1 -> Add simultaneous, 0 -> No simultaneous

pool_simultaneous_across_sets = 1; % attach simultaneous to all sets
...
% Set/order IDs.
% For the figure grouping, still use all condition set IDs from prefix/sim trials.
all_sets = unique(conditionSetID_trial(conditionType_trial > 0 & conditionSetID_trial > 0));
...
fprintf('Include simultaneous reference: %d\n', include_simultaneous_reference);
...
%   optional one extra subplot for simultaneous
%
% Bottom subplot:
%   overlay PSTH curves for all prefixes + optional simultaneous
        ...
        for i_amp = 1:numel(amps_sel)
            ...
            for i_isi = 1:numel(isi_sel)
                ...
                for ip = 1:numel(prefix_sel)
                    cond_i = cond_i + 1;
                    plotConds(cond_i).type = 'prefix';
                    plotConds(cond_i).prefix = prefix_sel(ip);
                    plotConds(cond_i).label = sprintf('Prefix %d', prefix_sel(ip));
                end

                % Optional simultaneous reference.
                if include_simultaneous_reference == 1
                    cond_i = cond_i + 1;
                    plotConds(cond_i).type = 'sim';
                    plotConds(cond_i).prefix = NaN;

                    if pool_simultaneous_across_sets == 1
                        plotConds(cond_i).label = 'Simultaneous pooled';
                    else
                        plotConds(cond_i).label = 'Simultaneous';
                    end
                end
                ...
                for cc = 1:nRaster
                    ...
                    if strcmp(plotConds(cc).type, 'prefix')

                        prefix_val = plotConds(cc).prefix;
                    ...
                    elseif strcmp(plotConds(cc).type, 'sim')

                        % ================= XIA MODIFICATION =================
                        % Simultaneous reference does not use ISI.
                        % If pooling is enabled, do not restrict by set_id.
                        if pool_simultaneous_across_sets == 1
                            tlist_check = find(conditionType_trial == 2 & ...
                                               trialAmps == amp_val);
                        else
                            tlist_check = find(conditionType_trial == 2 & ...
                                               conditionSetID_trial == set_id & ...
                ...
                for cc = 1:nRaster
                    ...
                    if strcmp(plotConds(cc).type, 'prefix')
                        ...
                        title_prefix = sprintf('Prefix %d', prefix_val);
                    ...
                    elseif strcmp(plotConds(cc).type, 'sim')
                        ...
                        % ISI is ignored.
                        %
                        % If pool_simultaneous_across_sets == 1, attach all
                        % simultaneous trials at this amplitude to every set
                        % figure, because simultaneous has no electrode order.
                        if pool_simultaneous_across_sets == 1
                            tlist = find(conditionType_trial == 2 & ...
                                         trialAmps == amp_val);

                            title_prefix = 'Simultaneous pooled';
                        else
                            tlist = find(conditionType_trial == 2 & ...
                                         conditionSetID_trial == set_id & ...
                                         trialAmps == amp_val);

                            title_prefix = 'Simultaneous';
                        end
                        % =====================================================
                    ...
                    if isempty(tlist)
                        ...
                        title(axR, sprintf('%s | No trials', title_prefix), ...
                              'Interpreter', 'none');

                        psth_all{cc} = zeros(1, numel(ctrs));
                        psth_labels{cc} = title_prefix;
                        ptd_this = NaN;
                        ...
                        %% ----- Raster title -----
                        title(axR, sprintf('%s | Final artifact %.1f ms | %d trials', ...
                                           title_prefix, ptd_this, numel(tlist)), ...
                                           'Interpreter', 'none');
                    end
                ...
                if include_simultaneous_reference == 1
                    if pool_simultaneous_across_sets == 1
                        sgtitle(sprintf('Rec Ch %d | Set %d: %s | %g uA | Seq ISI %.1f ms', ...
                            ch_plot, set_id, set_label, amp_val, isi_val), ...
                            'Interpreter', 'none');
                    else
                        sgtitle(sprintf('Rec Ch %d | Set %d: %s | %g uA | Seq ISI %.1f m', ...
                            ch_plot, set_id, set_label, amp_val, isi_val), ...
                            'Interpreter', 'none');
                    end
                else
