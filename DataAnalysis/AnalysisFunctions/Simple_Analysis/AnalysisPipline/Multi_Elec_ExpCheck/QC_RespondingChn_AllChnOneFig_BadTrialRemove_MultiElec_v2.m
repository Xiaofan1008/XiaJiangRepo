%% =============================================================
%  Cleaned Raster + PSTH All-Channel Viewer
%  for Mixed-Prefix Multi-Electrode Stimulation
%
%  Purpose:
%    Plot all recording channels in one figure for each condition.
...
raster_chn_end   = 25;      % Depth_s index
...
%% ----- Optional diagnostic: show amplitudes stored in Responding -----
if hasResp
    fprintf('\nResponding file amplitude values by set:\n');

    for ss_tmp = 1:numel(Resp.set)

        if isfield(Resp.set(ss_tmp), 'setID')
            thisSetID = Resp.set(ss_tmp).setID;
        else
            thisSetID = ss_tmp;
        end
        ...
        if isfield(Resp.set(ss_tmp), 'amp')
            for aa_tmp = 1:numel(Resp.set(ss_tmp).amp)
                if isfield(Resp.set(ss_tmp).amp(aa_tmp), 'amp_value')
                    ampList(end+1) = Resp.set(ss_tmp).amp(aa_tmp).amp_value; %#ok<SAGROW>
                elseif isfield(Resp.set(ss_tmp).amp(aa_tmp), 'ampVal')
                    ampList(end+1) = Resp.set(ss_tmp).amp(aa_tmp).ampVal; %#ok<SAGROW>
                elseif isfield(Resp.set(ss_tmp).amp(aa_tmp), 'amplitude')
                    ampList(end+1) = Resp.set(ss_tmp).amp(aa_tmp).amplitude; %#ok<SAGROW>
                end
            end
        end

        fprintf('  Set %d: ', thisSetID);
        disp(ampList);
    end
end
    ...
    for ai = 1:numel(Plot_Amps_selected)
        ...
        if ismember(2, ConditionTypes_selected)
            ...
            if pool_simultaneous_across_sets
                ...
                % Note:
                % If simultaneous is pooled across several sets,
                % responding highlight becomes ambiguous.
                % For clean interpretation, use pool_simultaneous_across_sets = false.
            ...
            for idxDepth = 1:nChPlot
                ...
                if highlight_responding_channels && hasResp
                    ...
                    % For non-pooled simultaneous trials, setID is the true sim set.
                    % For pooled simultaneous trials, this uses the current setID,
                    % but the interpretation is less clean.
                    respSetForSim = setID;
...
function si = find_resp_set_index(Resp, targetSetID)
    ...
    for k = 1:numel(Resp.set)
        ...
        if isfield(Resp.set(k), 'setID')
            ...
            % Fallback for older structures where setID was not stored.
            if k == targetSetID
                si = k;
                return;
            end
...
function pi = find_resp_prefix_index(prefixStruct, targetPrefix, targetISI)
    ...
    for k = 1:numel(prefixStruct)
        ...
        elseif isfield(P, 'ISI_ms')
            ...
            % If ISI was not stored in Responding, accept prefix match.
            isiVal = targetISI;
        end
