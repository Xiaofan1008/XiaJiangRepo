...
%
%  Important:
%       This code does NOT use PTD to define sim/seq.
%       This is necessary because Prefix 1 sequential can also have PTD = 0.
%
%  Normalization:
%       For each sequential set comparison:
%
%       NormFactor_SetX =
%           max(Seq SetX at NormRefAmp, Sim reference at NormRefAmp)
%
%       Then:
%           SCn_seq(:, :, SetX)     = SC_seq(:, :, SetX) / NormFactor_SetX
%           SCn_sim_ref(:, :, SetX) = Sim reference / NormFactor_SetX
%
%       In normalized plot:
%           same colour = same set comparison
%           solid line  = sequential
%           dashed line = simultaneous reference
% ============================================================
...
% Normalize each sequential set using its matched comparison:
%
%   NormFactor_SetX = max(Seq SetX at NormRefAmp, Sim reference at NormRefAmp)
%
% Then:
%   SCn_seq(:, :, SetX)     = SC_seq(:, :, SetX) / NormFactor_SetX
%   SCn_sim_ref(:, :, SetX) = Sim reference curve / NormFactor_SetX
%
% In normalized plot:
%   same colour = same set comparison
%   solid line  = sequential
...
NormalizeSpikeCount = true;

NormRefAmp = 5;                  % reference amplitude for normalization
NormUseInterpolation = true;     % use interpolation if exact ref amp is missing
NormAllowExtrapolation = false;  % safer: no extrapolation
MinNormFactor = 0.05;            % avoid unstable huge normalized values

% If empty, the code automatically uses the first set that has simultaneous data.
% If you want to force a particular simultaneous reference set, e.g. Set 1:
% SimReferenceSetID = 1;
SimReferenceSetID = [];
...
%% =============================================================
%  NORMALIZATION: MATCHED SEQ SET VS SIM REFERENCE
%% =============================================================

SCn_seq = nan(size(SC_seq));

% This stores the same raw simultaneous reference, but normalized separately
% for each sequential set comparison.
SCn_sim_ref = nan(size(SC_seq));

Norm = struct();
Norm.Method = 'per_set_max_seq_and_sim_ref_at_ref_amp';
Norm.RefAmp = NormRefAmp;
Norm.UseInterpolation = NormUseInterpolation;
Norm.AllowExtrapolation = NormAllowExtrapolation;
Norm.MinNormFactor = MinNormFactor;

Norm.Factor = nan(1, nSet);
Norm.SeqRefValue = nan(1, nSet);
Norm.SimRefValue = nan(1, nSet);
Norm.SimReferenceSetID = nan(1, nSet);
Norm.SimReferenceSetIndex = nan(1, nSet);
Norm.InterpolationUsedSeq = false(1, nSet);
Norm.InterpolationUsedSim = false(1, nSet);
Norm.Valid = false(1, nSet);

if NormalizeSpikeCount
    ...
    if isempty(sim_sets_available)
        ...
        warning('No simultaneous data found. Normalization cannot be performed.');
        ...
        if isempty(SimReferenceSetID)
            ...
            fprintf('\nNormalization: using Set %d as automatic simultaneous reference.\n', ...
            ...
            if ~ismember(SimReferenceSetID_used, sim_sets_available)
                warning('Requested SimReferenceSetID = %d has no simultaneous data. Normalization may fail.', ...
                    SimReferenceSetID_used);
            end
        end

        simRefIdx = find(SetIDs == SimReferenceSetID_used, 1);

        if isempty(simRefIdx)
            warning('SimReferenceSetID %d not found in selected SetIDs. Normalization cannot be performed.', ...
                SimReferenceSetID_used);
        else

            %% ----- Mean simultaneous reference curve -----
            SimRefMeanCurve = mean(SC_sim(:, :, simRefIdx), 1, 'omitnan');
            SimRefMeanCurve = squeeze(SimRefMeanCurve);

            %% ----- Loop through each sequential set comparison -----
            for ss = 1:nSet

                setID = SetIDs(ss);

                SeqMeanCurve = mean(SC_seq(:, :, ss), 1, 'omitnan');
                SeqMeanCurve = squeeze(SeqMeanCurve);

                %% ----- Get Seq reference value at NormRefAmp -----
                [SeqRefValue, usedInterpSeq] = get_ref_value_from_curve( ...
                    Amps, SeqMeanCurve, NormRefAmp, ...
                    NormUseInterpolation, NormAllowExtrapolation);

                %% ----- Get Sim reference value at NormRefAmp -----
                [SimRefValue, usedInterpSim] = get_ref_value_from_curve( ...
                    Amps, SimRefMeanCurve, NormRefAmp, ...
                    NormUseInterpolation, NormAllowExtrapolation);

                Norm.SeqRefValue(ss) = SeqRefValue;
                Norm.SimRefValue(ss) = SimRefValue;
                Norm.SimReferenceSetID(ss) = SimReferenceSetID_used;
                Norm.SimReferenceSetIndex(ss) = simRefIdx;
                Norm.InterpolationUsedSeq(ss) = usedInterpSeq;
                Norm.InterpolationUsedSim(ss) = usedInterpSim;

                %% ----- Matched normalization factor -----
                normFactor = max([SeqRefValue, SimRefValue], [], 'omitnan');

                if isempty(normFactor) || isnan(normFactor) || normFactor < MinNormFactor

                    fprintf(['WARNING: Set %d normalization invalid. ', ...
                             'SeqRef=%.4f, SimRef=%.4f, NormFactor=%.4f\n'], ...
                             setID, SeqRefValue, SimRefValue, normFactor);

                    continue;
                end

                Norm.Factor(ss) = normFactor;
                Norm.Valid(ss) = true;

                %% ----- Apply matched normalization -----
                SCn_seq(:, :, ss) = SC_seq(:, :, ss) ./ normFactor;

                % Important:
                % The simultaneous reference is duplicated for each set comparison,
                % but each copy is normalized by that set's own NormFactor.
                SCn_sim_ref(:, :, ss) = SC_sim(:, :, simRefIdx) ./ normFactor;

                fprintf(['Normalization Set %d: NormFactor=%.4f | ', ...
                         'SeqRef=%.4f | SimRef=%.4f | SimRefSet=%d\n'], ...
                         setID, normFactor, SeqRefValue, SimRefValue, SimReferenceSetID_used);
            end
        end
    end
    ...
    fprintf('\nNormalizeSpikeCount = false. Normalized arrays are left as NaN.\n');
end
...
if NormalizeSpikeCount
    ...
    for ss = 1:nSet
        ...
        for ai = 1:nAmp
            ...
            %% ----- Sequential normalized summary -----
            d_seq_n = squeeze(SCn_seq(:, ai, ss));
            d_seq_n = d_seq_n(~isnan(d_seq_n));

            if ~isempty(d_seq_n)

                normRowCounter = normRowCounter + 1;

                NormSummaryRows(normRowCounter,:) = { ...
                    setID, ...
                    setLabel, ...
                    'Sequential', ...
                    ampVal, ...
                    ...
                    NaN, ...
                    TargetSeqISI_ms, ...
                    Norm.SimReferenceSetID(ss), ...
                    Norm.Factor(ss), ...
                    Norm.SeqRefValue(ss), ...
                    Norm.SimRefValue(ss), ...
                    mean(d_seq_n, 'omitnan'), ...
                    std(d_seq_n, 0, 'omitnan') ./ sqrt(sum(~isnan(d_seq_n)))};
            end

            %% ----- Matched simultaneous reference normalized summary -----
            d_sim_n = squeeze(SCn_sim_ref(:, ai, ss));
            d_sim_n = d_sim_n(~isnan(d_sim_n));
            ...
            if ~isempty(d_sim_n)
                ...
                NormSummaryRows(normRowCounter,:) = { ...
                    ...
                    Norm.SeqRefValue(ss), ...
                    Norm.SimRefValue(ss), ...
                    mean(d_sim_n, 'omitnan'), ...
                    std(d_sim_n, 0, 'omitnan') ./ sqrt(sum(~isnan(d_sim_n)))};
            end
        end
    end
end

if isempty(NormSummaryRows)

    NormSummaryTable = table();

else

    NormSummaryTable = cell2table(NormSummaryRows, ...
        'VariableNames', { ...
        'SetID', ...
        'SetLabel', ...
        'Condition', ...
...
%% =============================================================
%  RAW PLOT: ONE LINE PER SEQUENTIAL SET AND RAW SIM SET
%% =============================================================

figure('Color','w', 'Position', fig_position);
hold on;

set_colors = lines(max(nSet, 1));
...
title(sprintf('Raw Spike Count: Seq Prefix %d, ISI %.1f ms vs Sim ActiveCount %d', ...
...
box off;
%% =============================================================
%  NORMALIZED PLOT: MATCHED SET PAIRS
%  Same colour = same set comparison
%  Solid line  = sequential
%  Dashed line = matched simultaneous reference
%% =============================================================

if NormalizeSpikeCount && any(Norm.Valid)
    ...
    for ss = 1:nSet
        ...
        setID = SetIDs(ss);
        setLabel = get_set_label(CHN, setID);
        simRefSetID = Norm.SimReferenceSetID(ss);

        col = set_colors(ss, :);

        %% ----- Sequential normalized -----
        data_seq_n = squeeze(SCn_seq(:, :, ss));

        if ~all(isnan(data_seq_n), 'all')

            AvgSeqN = mean(data_seq_n, 1, 'omitnan');
            SEMSeqN = std(data_seq_n, 0, 1, 'omitnan') ./ sqrt(sum(~isnan(data_seq_n), 1));

            for ai = 1:nAmp
                valid = ~isnan(data_seq_n(:, ai));

                if any(valid)
                    x_jit = (rand(sum(valid),1) - 0.5) * jitter_width + Amps(ai);

                    scatter(x_jit, data_seq_n(valid, ai), dot_size, col, ...
                        'filled', ...
                        'MarkerFaceAlpha', scatter_alpha, ...
                        'MarkerEdgeAlpha', scatter_alpha, ...
                        'HandleVisibility','off');
            ...
            plot_shaded_error(Amps, AvgSeqN, SEMSeqN, col);
        ...
        %% ----- Matched simultaneous reference normalized -----
        data_sim_ref_n = squeeze(SCn_sim_ref(:, :, ss));

        if ~all(isnan(data_sim_ref_n), 'all')

            AvgSimN = mean(data_sim_ref_n, 1, 'omitnan');
            SEMSimN = std(data_sim_ref_n, 0, 1, 'omitnan') ./ sqrt(sum(~isnan(data_sim_ref_n), 1));

            plot_shaded_error(Amps, AvgSimN, SEMSimN, col);

            plot(Amps, AvgSimN, '--o', ...
                'Color', col, ...
                'LineWidth', 2.2, ...
                'MarkerFaceColor', col, ...
                'DisplayName', sprintf('Sim ref for Set %d (Sim Set %d)', setID, simRefSetID));
        end
    end
    ...
    xlabel('Amplitude (\muA)', 'FontWeight','bold');
    ylabel(sprintf('Normalized spike count / trial\nrelative to max(Seq, Sim ref) at %.1f \\muA', NormRefAmp), ...
        'FontWeight','bold');

    title(sprintf('Normalized Spike Count: Seq Prefix %d, ISI %.1f ms vs Sim Reference', ...
        TargetSeqPrefix, TargetSeqISI_ms), ...
        'FontWeight','bold');
    ...
    fprintf('\nNormalized plot skipped: normalization was disabled or invalid.\n');
...
if numel(sim_sets_available) == 1
    ...
    fprintf('Common-reference comparison is exploratory and may include set/location/order effects.\n');
end
...
Result.Info.NormalizeSpikeCount = NormalizeSpikeCount;
Result.Info.NormRefAmp = NormRefAmp;
Result.Info.NormUseInterpolation = NormUseInterpolation;
Result.Info.NormAllowExtrapolation = NormAllowExtrapolation;
Result.Info.MinNormFactor = MinNormFactor;
Result.Info.SimReferenceSetID_requested = SimReferenceSetID;

Result.Data = struct();
Result.Data.SC_seq = SC_seq;
Result.Data.SC_sim = SC_sim;
Result.Data.SCn_seq = SCn_seq;
Result.Data.SCn_sim_ref = SCn_sim_ref;
...
Result.Summary.NormTable = NormSummaryTable;
...
out_filename = fullfile(save_dir, ...
    sprintf('Result_SpikeCount_MixedPrefix_Norm_%s.mat', base_name));
...
function [refValue, usedInterpolation] = get_ref_value_from_curve( ...
    Amps, MeanCurve, RefAmp, UseInterpolation, AllowExtrapolation)
    ...
    valid = ~isnan(MeanCurve) & ~isnan(Amps);
    ...
    Curve_valid = MeanCurve(valid);

    %% ----- First try exact match -----
    exact_idx = find(abs(Amps_valid - RefAmp) < 1e-6, 1);

    if ~isempty(exact_idx)
        refValue = Curve_valid(exact_idx);
        usedInterpolation = false;
        return;
    end

    %% ----- If no exact match, optionally interpolate -----
    if ~UseInterpolation
        return;
    end

    if numel(Amps_valid) < 2
        return;
    end

    %% ----- Avoid extrapolation unless explicitly allowed -----
    if ~AllowExtrapolation
        if RefAmp < min(Amps_valid) || RefAmp > max(Amps_valid)
            return;
        end

        refValue = interp1(Amps_valid, Curve_valid, RefAmp, 'linear');
        usedInterpolation = true;

    else

        refValue = interp1(Amps_valid, Curve_valid, RefAmp, 'linear', 'extrap');
        usedInterpolation = true;
    end
end