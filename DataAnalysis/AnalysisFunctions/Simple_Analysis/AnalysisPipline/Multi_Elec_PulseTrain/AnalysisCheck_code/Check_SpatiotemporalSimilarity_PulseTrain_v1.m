%% ============================================================
%  Quick Spatiotemporal Pattern Similarity Checker
%  for Pulse-Train Recovery Data
%
%  Purpose:
%    Test whether sequential pulse-train stimulation produces more distinct
%    spatiotemporal response patterns than AutoSim/simultaneous pulse train.
% ============================================================
...
% Spatiotemporal analysis window.
%
% For pulse trains, [0 60] is a good first choice.
% Sequential final event is around 25 ms.
% AutoSim final event is around 20 ms.
analysis_window_ms = [0 60];

% Time bin size.
%
% 2 ms gives fine temporal resolution.
% 5 ms gives smoother and more stable patterns.
%
...
% Optional smoothing along time dimension.
%
% 0 = no smoothing.
...
%
% For pattern testing, I recommend false first.
% We want to compare Seq Set2 and Seq Set3 separately.
pool_seq_sets = false;

% Pool AutoSim sets?
pool_autosim_sets = false;

% Normalise each spatiotemporal vector before distance calculation?
%
% false:
%   similarity includes both magnitude and pattern.
%
% true:
%   vector is divided by its norm before calculating distance.
%   This focuses more on pattern shape than total response magnitude.
%
% I recommend:
%   false first
%   then true as sensitivity check
normalise_vectors_before_distance = false;

% For visualisation only:
% use the same colour scale across all condition matrices for each amplitude.
use_same_caxis_across_conditions = true;
...
fprintf('\n================ Conditions for spatiotemporal comparison ================\n');
...
%% ====================== BUILD SPATIOTEMPORAL RESPONSE MATRICES ======================
% STMat dimensions:
...
%
% Then for similarity:
%   each condition matrix is flattened into one vector.

nCond = numel(CondList);
nAmp = numel(Amps_selected);

STMat = NaN(nAnalyseCh, nBins, nCond, nAmp);
STSEMMat = NaN(nAnalyseCh, nBins, nCond, nAmp);
STNMat = NaN(nAnalyseCh, nBins, nCond, nAmp);
    ...
    for aa = 1:nAmp
        ...
        for ich_i = 1:nAnalyseCh
            ...
            for tt = 1:numel(trial_list)
                ...
                % Baseline count for this trial/channel.
            ...
            channel_time_sem    = std(trial_bin_counts, 0, 1, 'omitnan') ./ sqrt(sum(~isnan(trial_bin_counts),1));

            % Optional smoothing along time.
            if smooth_bins > 1
                channel_time_vector = movmean(channel_time_vector, smooth_bins, 'omitnan');
                channel_time_sem    = movmean(channel_time_sem, smooth_bins, 'omitnan');
            end

            STMat(ich_i,:,c,aa) = channel_time_vector;
            STSEMMat(ich_i,:,c,aa) = channel_time_sem;
            STNMat(ich_i,:,c,aa) = sum(~isnan(trial_bin_counts),1);
        end
...
fprintf('\n================ Spatiotemporal Pattern Summary ================\n');
disp(SummaryTable);
fprintf('================================================================\n');

%% ====================== SPATIOTEMPORAL SIMILARITY ANALYSIS ======================
...
for aa = 1:nAmp
    ...
    for i = 1:nCond
        ...
        M1 = squeeze(STMat(:,:,i,aa));
        v1 = M1(:);

        for j = 1:nCond

            M2 = squeeze(STMat(:,:,j,aa));
            v2 = M2(:);
    ...
    for i = 1:nCond-1
        for j = i+1:nCond
            ...
            M_i = squeeze(STMat(:,:,i,aa));
            M_j = squeeze(STMat(:,:,j,aa));
            
            v_i = M_i(:);
            v_j = M_j(:);
            
            nValidFeatures = sum(~isnan(v_i) & ~isnan(v_j));
...
fprintf('\n================ Pairwise Spatiotemporal Pattern Similarity ================\n');
disp(PairwiseTable);
fprintf('=========================================================================\n');

%% ====================== FIGURE 1: SPATIOTEMPORAL MATRICES ======================
...
for aa = 1:nAmp
    ...
    % Determine common colour scale for this amplitude.
    if use_same_caxis_across_conditions
        allVals = [];
        for c = 1:nCond
            thisM = squeeze(STMat(:,:,c,aa));
            allVals = [allVals; thisM(:)]; %#ok<AGROW>
        end
        ...
        if isempty(allVals)
            ...
            if maxAbs == 0
                ...
                clim_this = [-maxAbs maxAbs];
            end
        end
    end

    figure('Color','w', ...
           'Name', sprintf('SpatiotemporalPattern_Matrices_Amp%g', amp_val), ...
           'Position', [80 80 1200 450]);
    ...
    for c = 1:nCond
        ...
        ax = nexttile;
        M = squeeze(STMat(:,:,c,aa));

        imagesc(ax, bin_centres, 1:nAnalyseCh, M);
        axis(ax, 'tight');
        ...
        yticks(ax, 1:nAnalyseCh);
        yticklabels(ax, arrayfun(@num2str, channels_to_analyse, 'UniformOutput', false));

        xlabel(ax, 'Time after trigger (ms)');
        ylabel(ax, 'Recording channel (Depth_s)');

        title(ax, CondList(c).ShortLabel, 'Interpreter','none');

        % Mark stimulation event times if available.
        tr_example = CondList(c).TrialList(1);
        evt = eventTimes_ms_trial{tr_example};

        hold(ax, 'on');
        if ~isempty(evt)
            for ee = 1:numel(evt)
                xline(ax, evt(ee), 'w--', 'LineWidth', 0.8);
            end
        end
    end

    sgtitle(sprintf('Spatiotemporal response matrix | %.1f \\muA | bin %.1f ms', ...
        amp_val, bin_ms), ...
        'Interpreter','none');
end
...
%
% Value:
%   Pearson correlation between flattened channel x time matrices.
%
% Interpretation:
%   Higher value = more similar spatiotemporal pattern.
%   Lower value  = more distinct spatiotemporal pattern.
...
for aa = 1:nAmp
    ...
    figure('Color','w', ...
           'Name', sprintf('SpatiotemporalPattern_PearsonSimilarity_Amp%g', amp_val), ...
    ...
    title(sprintf('Spatiotemporal Pearson similarity | %.1f \\muA', amp_val), ...
        'Interpreter','none');
...
%% ====================== FIGURE 3: PAIRWISE DISTANCE BAR PLOT ======================
% One figure per amplitude.
%
% Metric:
...
%
% Interpretation:
%   Larger distance = more different spatiotemporal response pattern.
...
for aa = 1:nAmp
    ...
    figure('Color','w', ...
           'Name', sprintf('SpatiotemporalPattern_PairwiseDistance_Amp%g', amp_val), ...
    ...
    ylabel('Spatiotemporal pattern distance (1 - Pearson r)');
    xlabel('Condition pair');

    title(sprintf('Pairwise spatiotemporal pattern distance | %.1f \\muA | Larger = more distinct', ...
        amp_val), ...
        'Interpreter','none');
...
if save_similarity_result
    ...
    out_name = sprintf('%s_SpatiotemporalPatternSimilarity_%s_Bin%.0fms_Resp%.0fto%.0fms.mat', ...
    ...
    fprintf('\nSaved spatiotemporal-pattern similarity result to:\n%s\n', fullfile(data_folder, out_name));
end

fprintf('\nFinished spatiotemporal pattern similarity analysis.\n');
