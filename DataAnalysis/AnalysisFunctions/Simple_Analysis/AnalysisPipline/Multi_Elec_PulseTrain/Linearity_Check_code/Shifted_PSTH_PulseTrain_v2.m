...
psth_baseline_mode = 'none';       % 'none', 'subtract', or 'subtract_clamp0'
...
for aa = 1:numel(Amps_to_plot)
    ...
    for cc = 1:numel(channels_to_plot)
        ...
        [SingleFirst_mean, SingleFirst_sem] = buildConditionPSTH( ...
            ...
            bin_edges, baseline_window_ms, smooth_bins, psth_baseline_mode);
        ...
        figTitleAuto = sprintf('AutoSim prediction check | %.1f uA | Depth_s Ch%d | %s | baseline=%s', ...
            amp_val, depth_ch, autoLabel, psth_baseline_mode);
        ...
        figTitleSeq1 = sprintf('Seq %s prediction check | %.1f uA | Depth_s Ch%d | baseline=%s', ...
            seqFirstSecondLabel, amp_val, depth_ch, psth_baseline_mode);
        ...
        figTitleSeq2 = sprintf('Seq %s prediction check | %.1f uA | Depth_s Ch%d | baseline=%s', ...
            seqSecondFirstLabel, amp_val, depth_ch, psth_baseline_mode);
...
function [psth_mean, psth_sem] = buildConditionPSTH( ...
    Data, condID, spike_ch, bin_edges, baseline_window_ms, smooth_bins, psth_baseline_mode)
...
%   psth_sem  = SEM across trials per bin
    ...
    for tt = 1:numel(trial_list)
        ...
        switch lower(psth_baseline_mode)
            ...
            case 'subtract_clamp0'
                ...
                trial_counts(tt,:) = corrected_counts;

            otherwise

                error('Unknown psth_baseline_mode: %s', psth_baseline_mode);
        end
    end
...
function plotAutoSimRasterPSTHCheck( ...
    ...
    % MODIFIED: PSTH only shows predicted vs actual.
    ...
    title(ax4, 'PSTH overlay: predicted vs actual', 'Interpreter','none');
...
function plotSeqRasterPSTHCheck( ...
    ...
    title(ax5, 'PSTH overlay: predicted vs actual', 'Interpreter','none');
