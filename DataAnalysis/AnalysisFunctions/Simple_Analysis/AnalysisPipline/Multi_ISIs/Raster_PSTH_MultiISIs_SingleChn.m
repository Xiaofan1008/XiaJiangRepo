...
% --- Analysis Settings ---
target_channels = [6:12];    % Choose which channel(s) to plot
...
% [MODIFIED] Added control to exclude bad trials
...
% [MODIFIED] Load Bad Trials if they exist
QC = struct();
QC.BadTrials = [];
f_bt = dir('*.MultiISIs_BadTrials.mat'); 
if ~isempty(f_bt)
    tmp_bt = load(f_bt(1).name); 
    QC.BadTrials = tmp_bt.BadTrials; 
end
...
for ch_idx = 1:length(target_channels)
    ...
    % [MODIFIED] Extract Bad Trials specifically for this channel
    bad_trs = []; 
    if exclude_bad_trials && ~isempty(QC.BadTrials) && target_channel <= length(QC.BadTrials)
        bad_trs = QC.BadTrials{target_channel}; 
    end
    ...
    for amp_val = plot_amps
        for s = 1:D_seq.nSets
            ...
            % Create Figure with Tiled Layout for Top/Bottom split
            ...
            for p_idx = 1:length(use_PTDs)
                ...
                % [MODIFIED] Remove bad trials before randomly selecting the plot trials
                trial_ids = setdiff(trial_ids, bad_trs);
                ...
                for tr = trial_ids'
                    ...
                    if ~isempty(tt)
                        % Plot dots directly onto ax_raster
                ...
                % Plot smoothed PSTH line directly onto ax_psth
                plot(ax_psth, ctrs, rate_seq, 'LineStyle', '-', 'Marker', 'none', 'Color', this_col, 'LineWidth', 2.5, 'DisplayName', sprintf('ISI %.0f ms', ptd_val));
