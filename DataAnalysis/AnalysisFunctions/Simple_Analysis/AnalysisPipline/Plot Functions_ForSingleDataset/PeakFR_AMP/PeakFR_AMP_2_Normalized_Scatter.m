...
%   Simultaneous vs Sequential (Averaged Orders)
%   + SCATTER PLOTS ENABLED
% ============================================================
clear;
...
% PSTH parameters
bin_ms     = 1;
sigma_bins = 5;        % Gaussian smoothing kernel (in bins)
...
% Plotting Aesthetics
jitter_width = 0.3;     % How much to spread dots horizontally (e.g. +/- 0.15 uA)
scatter_alpha = 0.4;    % Transparency of dots (0-1)
dot_size = 20;          % Size of scatter dots
...
if any(~isnan(y))
    ...
    % Loop through amplitudes to add jittered scatter
    for i = 1:length(x)
        amp_val = x(i);
        chan_vals = NormPeakFR_sim(:, i);
        
        % Generate Jitter: Random numbers between -jitter_width/2 and +jitter_width/2
...
% --- 2. Plot Sequential (Combined Order) ---
% Color map for multiple PTDs
seq_cols = cool(numel(PTDs_ms)+2); 

for p = 1:numel(PTDs_ms)
    x = Amps_seq;
    
    % Handle dimensions if 1 PTD vs multiple
    if numel(PTDs_ms) == 1
        y = AvgSeq_Pop(:)'; 
        se = AvgSeq_Pop_SEM(:)';
        col = [0.85 0.33 0.1]; % Red/Orange for single PTD
        
        % Data for Scatter: [Channel x Amp] (Squeezed for this PTD)
        scatter_data = NormPeakFR_seq_Combined; 
    else
        y = AvgSeq_Pop(:,p)';
        se = AvgSeq_Pop_SEM(:,p)';
        col = seq_cols(p+1,:);
        
        % Data for Scatter: [Channel x Amp] for this specific PTD
        scatter_data = squeeze(NormPeakFR_seq_Combined(:,:,p));
    end
...
function [peak_val, baseline_rates] = get_trial_data(tr_ids, trig, sp_data, ...
    ...
    all_post_spikes = []; % To store spike times from ALL trials
    for k = 1:nTr
        tr = tr_ids(k);
        t0 = trig(tr)/FS*1000;
        tt = sp_data(:,1) - t0;
        
        % 1. Baseline: Calculate rate per trial (needed for noise statistics)
        mask_b = tt >= base_win(1) & tt < base_win(2);
        baseline_rates(k) = sum(mask_b) / base_dur;
        
        % 2. Post-stim: Collect spikes for the aggregate PSTH
        tt_post = tt(tt >= post_win(1) & tt <= post_win(2));
        all_post_spikes = [all_post_spikes; tt_post]; 
    end
    % 3. Compute Peak from the AGGREGATE PSTH (Method B)
    if isempty(all_post_spikes)
        peak_val = 0;
    else
        % Histogram of all spikes
        h = histcounts(all_post_spikes, edges);
        
        % Normalize to rate:
        % Divide by (Number of Trials * Bin Size in Seconds)
...
function Z_val = normalize_data(peak_val, baseline_rates, k_MAD, epsilon, max_z)
    % 1. Calculate Baseline Stats from the vector of trial baselines
    medB = median(baseline_rates);
    MADB = median(abs(baseline_rates - medB));
    denom = k_MAD * MADB + epsilon;
    
    % 2. Calculate Z-score for the single peak value
    % (Peak - Median_Baseline) / Noise_Floor
