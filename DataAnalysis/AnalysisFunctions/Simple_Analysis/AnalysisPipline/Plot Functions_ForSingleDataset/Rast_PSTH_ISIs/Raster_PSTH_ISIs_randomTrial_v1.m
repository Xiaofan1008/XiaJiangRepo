...
%% ===================== USER INPUTS ===================== %%
% Removed Single folder, kept only Seq folder
...
% --- Analysis Settings ---
target_channels = [10:20];     % Choose which channel(s) to plot
plot_amps      = [10];       % Choose which Amplitude(s) to plot
target_PTDs    = [5 8 10 12 15 17 20 25]; % Only sequential ISIs > 0
% [MODIFIED] Set the number of random trials you want to plot per ISI
num_trials_plot = 20;        
...
% LOAD DATASETS
% ============================================================
% Removed Single Dataset loading block entirely.
% ---------- Load Combined (Seq) ----------
cd(folder_seq)
...
for ch_idx = 1:length(target_channels)
    ...
    for amp_val = plot_amps
        % Loop through Sets. Each Set gets its own layered figure.
        for s = 1:D_seq.nSets
            ...
            for p_idx = 1:length(use_PTDs)
                ...
                % [MODIFIED] Randomly select a specific number of trials
