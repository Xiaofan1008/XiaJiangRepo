...
%   - EXCLUDES GAPS (Measures "Net Firing Duration")
%   - Filter: Symmetric Gaussian (Zero-Phase)
% ============================================================
clear;
...
title(ax1, 'Net Active Time (Excluding Gaps)', 'FontWeight', 'bold');
...
% %% TWO-WAY ANOVA: Is Sequential Duration Significantly Different?
% % flatten the data into a table: [Duration, StimType, Amplitude]
% y_dur = [];
% g_stim = [];
% g_amp = [];
% 
...
% end
% 
% % 2. Collect Sequential Data (e.g., Set 2)
% % Note: Choose the specific Set you want to compare, or avg sets first
% target_set = 2; % Example: Comparing Seq Set 2 vs Sim
% for ai = 1:length(Amps_seq)
%     d = squeeze(Dur_seq(:, ai, target_set, 1)); 
%     y_dur = [y_dur; d];
...
% 
% % 3. Run ANOVA
% [p, tbl, stats] = anovan(y_dur, {g_stim, g_amp}, 'model', 'interaction', 'varnames', {'StimType', 'Amplitude'});
% 
% fprintf('\n=== ANOVA RESULTS ===\n');
...
%   Comparison: Simultaneous vs. Sequential (All Sets Combined)
% ============================================================
...
% --- 1. Collect Simultaneous Data ---
for ai = 1:length(Amps_sim)
    % Extract duration data for this amplitude
    d = Dur_sim(:, ai);
    
    % Remove NaNs (Crucial for correct group sizing)
    d = d(~isnan(d));
...
% --- 2. Collect Sequential Data (POOLED) ---
% Loop through ALL sets instead of picking just one
for ss = 1:nSets_seq
    for ai = 1:length(Amps_seq)
        % Extract duration: [Ch x Amp x Set x (Event)]
        % Assuming the 4th dim is the specific duration metric you want
        ...
        y_dur  = [y_dur; d];
        % Label them all as 'Sequential' to merge them into one group
        g_stim = [g_stim; repmat({'Sequential'}, length(d), 1)];
        g_amp  = [g_amp;  repmat(Amps_seq(ai), length(d), 1)];
    end
end
...
if ~isempty(y_dur)
    ...
    % Optional: Pairwise Comparison Plot
    figure('Color','w', 'Name', 'Duration Comparison');
    multcompare(stats, 'Dimension', 1);
    title('Response Duration: Simultaneous vs Sequential');
    ylabel('Stimulation Type'); xlabel('Mean Duration (ms)');
else
    fprintf('Error: No valid duration data found.\n');
end
...
function duration_ms = get_duration_sum(tr_ids, trig, sp_data, ...
    ...
    if isempty(baseline_part), sigma_base = 5; else, sigma_base = std(baseline_part); end
    sigma_base = max(sigma_base, 2.0); 
    threshold = base_rate_avg + z_thresh * sigma_base;
    
    % --- SUMMATION LOGIC (EXCLUDE GAPS) ---
    % Find all bins within search window that exceed threshold
    valid_mask = (t_centers >= search_win(1)) & (t_centers <= search_win(2));
    significant_mask = (rate_smooth > threshold) & valid_mask;
    
    % Count number of significant bins
    num_bins = sum(significant_mask);
    
    % Duration = Bins * Bin_Size
    duration_ms = num_bins * bin_ms;
    
    % Minimal check: if total duration < 2ms, ignore
