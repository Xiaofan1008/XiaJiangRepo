...
fprintf('Analyzing Population (Separated by Set for Sim & Seq 5ms):\n');
for ss = 1:nSets
    ...
    if ss <= numel(R.set)
        for ai=1:numel(R.set(ss).amp)
            for pi=1:numel(R.set(ss).amp(ai).ptd)
                ...
                curr_ptd = R.set(ss).amp(ai).ptd(pi).PTD_ms;
...
% [MODIFIED] Init Output Arrays based on nCh_Total so absolute channel indices align
...
% [MODIFIED] Loop through sets first, then process that set's responding channels
for ss = 1:nSets
    ...
    for ci = 1:length(current_set_channels)
        ...
        % Bad Channel Check specifically for this set
        is_bad_ch = false;
        if ~isempty(QC.BadCh) && ss <= length(QC.BadCh) && ismember(ch_idx, QC.BadCh{ss})
            is_bad_ch = true;
        end
        
        % [MODIFIED] If bad channel, skip early since matrices are pre-filled with NaN
        ...
        if ~isempty(ptd_sim_idx)
            for ai = 1:length(Amps)
                ...
                % [MODIFIED] Save using absolute ch_idx instead of relative ci
