...
for ci = 1:length(resp_channels)
    ...
    if ~isempty(ptd_sim_idx)
        for ss = 1:nSets
            ...
            for ai = 1:length(Amps)
                ...
                % [MODIFIED] No response check (Union logic)
    ...
    % 1. Create Mean Response Curves (Average across sets to get robust curve)
    % Sim Curve
    if ~isempty(ptd_sim_idx)
    ...
    % We need at least 2 points to interpolate, but interp1 handles single points if 'nearest' (optional)
    % Use 'linear', 'extrap' to estimate Ref_Amp even if outside range
    ...
    elseif sum(valid_sim) == 1
        est_sim = sim_curve_avg(valid_sim); % Only 1 point? Use it.
    end
    ...
    % Note: If extrapolation goes negative (rare but possible with noise), clamp to 0.
    ...
    if ~isnan(norm_denom) && norm_denom > min_ref_response
        ...
        % Fallback: If calculation fails (e.g. all NaNs), prevent div by zero
