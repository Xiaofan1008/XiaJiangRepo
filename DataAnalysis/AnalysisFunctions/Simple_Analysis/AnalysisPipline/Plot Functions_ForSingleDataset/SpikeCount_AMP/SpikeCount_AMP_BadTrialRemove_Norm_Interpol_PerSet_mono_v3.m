%% ============================================================
%   Response Magnitude: PER-SET NORMALIZATION (Monotonic Interp)
%   - Metric: Mean Spike Count (2-20ms)
%   - Logic: 
%       1. Loop Set-by-Set.
%       2. Identify Union of responding channels for THAT Set.
%       3. IF 5uA exists -> Use Real Value.
%       4. IF 5uA missing -> Interpolate BUT ensure value >= lower neighbor.
...
min_ref_response = 0.01; % Threshold to avoid dividing by 0.
...
% Check if Ref_Amp exists exactly in this dataset
...
for ss = 1:nSets
    ...
    for k = 1:length(local_resp_indices)
        ...
        % Store Raw Values Strictly
        Raw_Sim_All(ch_idx, :, ss) = curve_sim;
        Raw_Seq_All(ch_idx, :, ss) = curve_seq;
        
        % 2. Determine Denominator (Real or Interp)
        val_sim_ref = NaN;
        val_seq_ref = NaN;
        
        if has_ref_amp
            % Case A: 5uA exists -> Use it directly
            val_sim_ref = curve_sim(ref_amp_idx);
            val_seq_ref = curve_seq(ref_amp_idx);
        else
            % Case B: 5uA missing -> Interpolate with Monotonic Constraint
            
            % --- Sim Interpolation ---
            valid_sim = ~isnan(curve_sim);
            if sum(valid_sim) >= 2
                val_sim_ref = interp1(Amps(valid_sim), curve_sim(valid_sim), Ref_Amp, 'linear', 'extrap');
                % CONSTRAINT: Estimated 5uA cannot be lower than the max value of any amplitude < 5uA
                % This prevents "bumps" where 4uA > Estimated_5uA
                valid_amps = Amps(valid_sim);
                valid_vals = curve_sim(valid_sim);
                idx_lower  = find(valid_amps < Ref_Amp, 1, 'last'); % Closest amp below 5uA
                if ~isempty(idx_lower)
                    val_lower = valid_vals(idx_lower);
                    if val_sim_ref < val_lower
                        val_sim_ref = val_lower; % Clamp to plateau
                    end
                end
            ...
            if sum(valid_seq) >= 2
                ...
                % CONSTRAINT
                valid_amps = Amps(valid_seq);
                valid_vals = curve_seq(valid_seq);
                idx_lower  = find(valid_amps < Ref_Amp, 1, 'last');
                if ~isempty(idx_lower)
                    val_lower = valid_vals(idx_lower);
                    if val_seq_ref < val_lower
                        val_seq_ref = val_lower; % Clamp to plateau
                    end
                end
        ...
        % 3. Find Max Denominator (Global Max for this Set)
        % Clamp negative interpolations to 0
