...
%   - Logic: 
%       1. Loop Set-by-Set.
%       2. Identify Union of responding channels for THAT Set.
%       3. IF 5uA exists -> Use Real Value.
%       4. IF 5uA missing -> Interpolate.
%       5. Normalize (Divide by Max(Sim_5uA, Seq_5uA)).
%   - NEW RULE: Numerator (Plot Data) is forced to 0 if non-responsive at that Amp.
%               Denominator (Ref Data) uses REAL spikes regardless of responsiveness.
% ============================================================
clear;
...
for ss = 1:nSets
    ...
    for k = 1:length(local_resp_indices)
        ...
        if ~isempty(QC_Sim.BadCh)
             if iscell(QC_Sim.BadCh) && ss<=length(QC_Sim.BadCh) && ismember(ch_idx, QC_Sim.BadCh{ss}), is_bad=true; end
             if ~iscell(QC_Sim.BadCh) && ismember(ch_idx, QC_Sim.BadCh), is_bad=true; end
        end
        if ~isempty(QC_Seq.BadCh)
             if iscell(QC_Seq.BadCh) && ss<=length(QC_Seq.BadCh) && ismember(ch_idx, QC_Seq.BadCh{ss}), is_bad=true; end
             if ~iscell(QC_Seq.BadCh) && ismember(ch_idx, QC_Seq.BadCh), is_bad=true; end
        end
        if is_bad, continue; end
        
        % 1. Get Raw Curves (REAL COUNTS for Denominator)
        curve_sim_real = nan(1, length(Amps));
        curve_seq_real = nan(1, length(Amps));
        
        % Get Raw Curves (FORCED ZERO for Numerator)
        curve_sim_plot = nan(1, length(Amps));
        curve_seq_plot = nan(1, length(Amps));
        ...
        for ai = 1:length(Amps)
            % --- Sim ---
            % A. Calculate Real Spike Count (For Denominator)
            tr = setdiff(find(combClass_sim==ss & ampIdx_sim==ai), bt_sim);
            if ~isempty(tr)
                real_count = get_spike_count(tr, trig_sim, S_ch_sim, post_win_ms, FS);
                curve_sim_real(ai) = real_count;
                
                % B. Apply Force-Zero Rule (For Numerator/Plotting)
                is_resp = 0;
                try is_resp = Rsim.set(ss).amp(ai).ptd(1).channel(ch_idx).is_responsive; catch, end
                
                if is_resp
                    curve_sim_plot(ai) = real_count;
                else
                    curve_sim_plot(ai) = 0; % [MODIFIED] Force 0 if not responding
                end
            end
            
            % --- Seq ---
            if ~isempty(ptd5_seq)
                % A. Calculate Real Spike Count
                tr = setdiff(find(combClass_seq==ss & ptdIdx_seq==ptd5_seq & ampIdx_seq==ai), bt_seq);
                if ~isempty(tr)
                    real_count = get_spike_count(tr, trig_seq, S_ch_seq, post_win_ms, FS);
                    curve_seq_real(ai) = real_count;
                    
                    % B. Apply Force-Zero Rule
                    is_resp = 0;
                    try is_resp = Rseq.set(ss).amp(ai).ptd(ptd5_seq).channel(ch_idx).is_responsive; catch, end
                    
                    if is_resp
                        curve_seq_plot(ai) = real_count;
                    else
                        curve_seq_plot(ai) = 0; % [MODIFIED] Force 0 if not responding
                    end
                end
            end
        end
        
        Raw_Sim_All(ch_idx, :, ss) = curve_sim_plot; % Store the filtered ones for checking if needed
        Raw_Seq_All(ch_idx, :, ss) = curve_seq_plot;
        
        % 2. Determine Denominator (Using REAL counts)
        val_sim_ref = NaN;
        val_seq_ref = NaN;
        
        if has_ref_amp
            val_sim_ref = curve_sim_real(ref_amp_idx);
            val_seq_ref = curve_seq_real(ref_amp_idx);
        else
            valid_sim = ~isnan(curve_sim_real);
            if sum(valid_sim) >= 2
                val_sim_ref = interp1(Amps(valid_sim), curve_sim_real(valid_sim), Ref_Amp, 'linear', 'extrap');
            elseif sum(valid_sim) == 1
                val_sim_ref = curve_sim_real(valid_sim);
            end
            
            valid_seq = ~isnan(curve_seq_real);
            if sum(valid_seq) >= 2
                val_seq_ref = interp1(Amps(valid_seq), curve_seq_real(valid_seq), Ref_Amp, 'linear', 'extrap');
            elseif sum(valid_seq) == 1
                val_seq_ref = curve_seq_real(valid_seq);
            end
        end
        
        % 3. Normalize (Numerator = Plot Data (Filtered), Denom = Real Data)
        ...
        if ~isnan(denom) && denom > min_ref_response
            Norm_Sim(ch_idx, :, ss) = curve_sim_plot / denom; % [MODIFIED] Use Forced-Zero curve
            Norm_Seq(ch_idx, :, ss) = curve_seq_plot / denom; % [MODIFIED] Use Forced-Zero curve
        else
            % Fallback (Raw Forced-Zero)
            Norm_Sim(ch_idx, :, ss) = curve_sim_plot;
            Norm_Seq(ch_idx, :, ss) = curve_seq_plot;
        end
    end
end
...
%% ================= SAVE RESULTS =================
save_dir = '/Volumes/MACData/Data/Data_Xia/Analyzed_Results/SpikeCount/DX005/';
