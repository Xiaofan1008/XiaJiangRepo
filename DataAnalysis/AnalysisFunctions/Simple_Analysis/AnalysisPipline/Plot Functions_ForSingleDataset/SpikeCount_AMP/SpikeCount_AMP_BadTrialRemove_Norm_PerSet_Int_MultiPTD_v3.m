...
%% ================= USER SETTINGS ============================
data_folder = '/Volumes/MACData/Data/Data_Xia/DX016/Xia_Exp1_Seq_Full_3';
...
min_ref_response = 0.01; % Threshold to avoid dividing by 0. (If <0.01, treat as 0 response)
...
for ss = 1:nSets
    ...
    for k = 1:length(local_resp_indices)
        ...
        if ~isnan(denom) && denom > min_ref_response
            ...
            % Fallback Logic:
            % If the max response at 5uA is roughly 0 (noise), we cannot divide by it.
            % We keep the data as Raw, or set to 0? 
            % Usually, if reference is 0, normalization is undefined.
            % We set to RAW here to avoid losing data, but be aware these are likely small values.
...
out_filename = fullfile(save_dir, ['Result_SpikeNormGlobalRef_' num2str(Ref_Amp) 'uA_5ms_' exp_id '.mat']);
