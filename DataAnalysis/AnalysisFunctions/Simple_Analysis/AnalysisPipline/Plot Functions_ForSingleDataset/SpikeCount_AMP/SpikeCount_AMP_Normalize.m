...
%   - Logic: 
%       1. Calc raw spike counts.
%       2. Find Sim_Max for that channel.
%       3. If Sim_Max < 0.5, use 0.5 as the denominator (Regularization).
%       4. Normalize both Sim and Seq by this denominator.
...
% If Sim Max is below this (e.g., 0.5 spikes), we divide by 0.5 instead of the tiny value.
noise_floor = 0.1; 
...
for ci = 1:length(resp_channels)
    ...
    % Apply Noise Floor Logic:
    % Use the larger of (Actual Sim Max) OR (Noise Floor 0.5)
    denominator = max(sim_max, noise_floor);
    
    % Now we normalize everyone. 
    % Even if Sim=0, denominator is 0.5, so we get valid numbers (0/0.5 = 0).
    % If Sim=0 but Seq=10, we get 10/0.5 = 20. (Valid!)
...
out_filename = fullfile(save_dir, ['Result_Set1_SpikeNormSimMax_Floor_5ms_' exp_id '.mat']);
