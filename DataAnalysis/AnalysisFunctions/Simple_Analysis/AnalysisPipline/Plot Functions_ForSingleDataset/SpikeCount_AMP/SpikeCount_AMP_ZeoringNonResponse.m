...
%   - REFINEMENT: If channel is NOT responsive -> Count = 0 (Ignore noise)
% ============================================================
clear;
addpath(genpath('/Volumes/MACData/Data/Data_Xia/AnalysisFunctions'));
%% ================= USER SETTINGS ============================
folder_sim = '/Volumes/MACData/Data/Data_Xia/DX011/Xia_Exp1_Sim3';
folder_seq = '/Volumes/MACData/Data/Data_Xia/DX011/Xia_Exp1_Seq3_5ms';
Electrode_Type = 1;
...
% UNION: Channels that responded at least once in ANY condition
...
for ci = 1:length(resp_channels)
    ...
    for ai = 1:length(Amps_sim)
        % --- NEW RULE: Check Responsiveness ---
        is_resp = 0; 
        try is_resp = Rsim.set(1).amp(ai).ptd(1).channel(ch_idx).is_responsive; catch, end
        
        if ~is_resp
            SpikeCount_sim(ci,ai) = 0; % Force 0
            PSTH_Sim(ci, :, ai)   = zeros(1, nBins); % Flat line
            continue; 
        end
        % --------------------------------------
    ...
    for p = 1:numel(PTDs_ms)
        for ss = 1:nSets_seq
            for ai = 1:length(Amps_seq)
                ...
                if ~is_resp
                    SpikeCount_seq(ci,ai,ss,p) = 0; % Force 0
                    PSTH_Seq(ci, :, ai, ss, p) = zeros(1, nBins); % Flat line
                    continue; 
                end
                % --------------------------------------
...
title('Response Magnitude (Total Spike Count - Zeroed Non-Resp)', 'FontWeight','bold');
...
fprintf('Condition       \t');
for ai = 1:length(Amps_sim)
    fprintf('%.0fuA_Mean SEM\t', Amps_sim(ai));
end
fprintf('\n');
% 2. Simultaneous Row
...
out_filename = fullfile(save_dir, ['Result_Set1_SpikeCount_FixedPop_Zeroed_5ms_' exp_id '.mat']);
ResultFR = struct();
ResultFR.Metadata.Created = datestr(now);
ResultFR.Metadata.Metric = 'Mean Spike Count per Trial (Zeroed Non-Resp)';
