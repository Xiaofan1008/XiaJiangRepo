%% ============================================================
%   Spike Count Analysis (Dynamic Window Method)
%   - Metric: Mean Spike Count per Trial (Baseline Subtracted)
%   - Logic: Counts spikes in [start, end + ISI] to capture full response.
...
% Choose the BASE dynamic window [start, base_end]
% The actual window will be [start, base_end + ISI]
base_dynamic_win = [2, 15]; 

% [MODIFIED] Define the baseline window to calculate spontaneous rate
baseline_win_ms = [-50, 0];
...
% [MODIFIED] Find the index for the target amplitude to filter channels strictly
...
for ss = 1:nSets
    ...
    if ss <= numel(R.set)
        % [MODIFIED] Only loop through the specific target amplitude (ai_target)
    ...
    for ci = 1:length(current_set_channels)
        ...
        if ~isempty(ptd_sim_idx) && any(abs(target_ISIs - 0) < 0.001)
            for ai = 1:length(Amps)
                ...
                % Calculate dynamic window for 0ms (Simultaneous)
                current_win = [base_dynamic_win(1), base_dynamic_win(2) + 0];
                
                % [MODIFIED] Pass baseline_win_ms to the counting function
                [count_val, psth_curve] = get_spike_count_dynamic(tr_ids, trig, S_ch, ...
                    current_win, baseline_win_ms, edges_psth, g_sym, bin_s, FS);
        ...
        for p = 1:numel(PTDs_ms)
            ...
            for ai = 1:length(Amps)
                ...
                % Calculate dynamic window (Base End + ISI)
                current_win = [base_dynamic_win(1), base_dynamic_win(2) + PTDs_ms(p)];
...
% [MODIFIED] Updated Y label to reflect baseline subtraction
ylabel(sprintf('Spike Count (%.1f uA, Subtracted, Win: %.1f to %.1f+ISI ms)', target_Amp_Stats, base_dynamic_win(1), base_dynamic_win(2)), 'FontWeight','bold', 'FontSize', 12);
title('ISI Tuning Curve (Dynamic Window + Baseline Subtracted)', 'FontWeight','bold', 'FontSize', 14);
...
% [MODIFIED] Rename save file to reflect BaseSub
out_filename = fullfile(save_dir, ['Result_SpikeCount_DynWin_BaseSub_' char(isi_str) 'ms_' exp_id '.mat']);

ResultFR = struct();
ResultFR.Metadata.Created = datestr(now);
ResultFR.Metadata.Metric = 'Dynamic Window Spike Count (Baseline Subtracted)';
ResultFR.Metadata.DynamicBaseWin = base_dynamic_win;
...
%% ==================== HELPER FUNCTIONS =========================
% [MODIFIED] Reverted counting logic to evaluate dynamic window AND subtract baseline
function [corrected_count, psth_trace] = get_spike_count_dynamic(tr_ids, trig, sp_data, ...
    count_win, base_win, psth_edges, g_sym, bin_s, FS)
    
    nTr = numel(tr_ids);
    all_psth_spikes = [];
    total_spikes_in_window = 0;
    total_base_spikes = 0; % Track baseline noise spikes
    ...
    for k = 1:nTr
        ...
        % 1. Count spikes strictly inside the provided dynamic response window
        mask_count = (tt >= count_win(1) & tt <= count_win(2));
        total_spikes_in_window = total_spikes_in_window + sum(mask_count);
        
        % 2. Count spikes in the Baseline window
        mask_base = (tt >= base_win(1) & tt <= base_win(2));
        total_base_spikes = total_base_spikes + sum(mask_base);
        
        % 3. Collect spikes for PSTH Visual
        all_psth_spikes = [all_psth_spikes; tt(tt >= psth_edges(1) & tt <= psth_edges(end))];
    end
    
    if nTr > 0
        % Calculate raw per-trial averages
        raw_count = total_spikes_in_window / nTr; 
        base_count = total_base_spikes / nTr;
        
        % Calculate Baseline Firing Rate (Hz)
        base_dur_sec = (base_win(2) - base_win(1)) / 1000;
        base_rate_Hz = base_count / base_dur_sec;
        
        % Calculate Expected Noise for the dynamic window length
        count_dur_sec = (count_win(2) - count_win(1)) / 1000;
        expected_noise = base_rate_Hz * count_dur_sec;
        
        % Subtract to get Corrected Fair Count
        corrected_count = raw_count - expected_noise;
    else
        corrected_count = NaN;
    end
