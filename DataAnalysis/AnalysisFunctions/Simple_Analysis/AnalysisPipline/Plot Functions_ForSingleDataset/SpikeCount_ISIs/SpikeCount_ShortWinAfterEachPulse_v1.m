%% ============================================================
%   Spike Count Analysis (Method 2: Pulse-by-Pulse Fixed Window)
%   - Metric: Mean Spike Count per Trial (Dual Window for Seq)
%   - Logic: Counts spikes in [w1, w2] after EVERY pulse.
%   - FILTER: Extracts Sim (PTD=0) and Selected Seq ISIs
% ============================================================
clear;
...
% Choose the short window AFTER EACH PULSE
pulse_win_ms = [0, 4.5]; 
...
% [MODIFIED] Find the target amplitude index early to filter channels
...
% [MODIFIED] Store responding channels individually for each set
...
for ss = 1:nSets
    ...
    if ss <= numel(R.set)
        % [MODIFIED] Only evaluate responsiveness for the specific target amplitude
    ...
    % [MODIFIED] Print responding channel counts per set
...
% [MODIFIED] Init Output Arrays based on total channels to maintain indices
...
% [MODIFIED] Loop through Sets first, then process only that set's channels
for ss = 1:nSets
    ...
    for ci = 1:length(current_set_channels)
        ...
        if ~isempty(ptd_sim_idx) && any(abs(target_ISIs - 0) < 0.001)
            for ai = 1:length(Amps)
                ...
                % Pass pulse_win_ms and ISI=0 to dual counting function
                [count_val, psth_curve] = get_spike_count_dual(tr_ids, trig, S_ch, ...
                    pulse_win_ms, edges_psth, g_sym, bin_s, FS, 0);
        ...
        for p = 1:numel(PTDs_ms)
            ...
            for ai = 1:length(Amps)
                ...
                % Pass pulse_win_ms and actual ISI to dual counting function
                [count_val, psth_curve] = get_spike_count_dual(tr_ids, trig, S_ch, ...
                    pulse_win_ms, edges_psth, g_sym, bin_s, FS, PTDs_ms(p));
...
ylabel(sprintf('Spike Count (%.1f uA, Win: %.1f-%.1f ms)', target_Amp_Stats, pulse_win_ms(1), pulse_win_ms(2)), 'FontWeight','bold', 'FontSize', 12);
title('ISI Tuning Curve (Pulse-by-Pulse Method)', 'FontWeight','bold', 'FontSize', 14);
...
out_filename = fullfile(save_dir, ['Result_SpikeCount_FixedPulseWin_' char(isi_str) 'ms_' exp_id '.mat']);

ResultFR = struct();
ResultFR.Metadata.Created = datestr(now);
ResultFR.Metadata.Metric = 'Pulse-by-Pulse Spike Count';
ResultFR.Metadata.PulseWindow = pulse_win_ms;
...
%% ==================== HELPER FUNCTIONS =========================
function [count_val, psth_trace] = get_spike_count_dual(tr_ids, trig, sp_data, ...
    pulse_win, psth_edges, g_sym, bin_s, FS, ISI_ms)
    ...
    for k = 1:nTr
        ...
        % 1. Count spikes for Pulse 1
        mask_count = (tt >= pulse_win(1) & tt <= pulse_win(2));
        
        % 2. Count spikes for Pulse 2 (Only if it's Sequential)
        if ISI_ms > 0
            mask_count = mask_count | (tt >= (ISI_ms + pulse_win(1)) & tt <= (ISI_ms + pulse_win(2)));
        end
