...
stats_target_Amp = 5; % Focus the Plot and Stats on this specific Amplitude
...
for k = 1:nFiles
    ...
    % [MODIFIED] Normalize to 10 uA Maximum (across ALL valid Sets to maintain relative scale)
    idx_10uA = find(abs(Union_Amps - 10) < 0.001);
    
    if ~isempty(idx_10uA)
        % Extract all data for 10uA across all ISIs and BOTH Sets for this animal
        ...
        if ~isempty(valid_10uA)
            ...
            if Max_10uA > 0
                ...
                fprintf('    -> Warning File %d: 10uA max is 0. Skipping normalization.\n', k);
            end
        else
            fprintf('    -> Warning File %d: No valid 10uA data found. Excluded from average.\n', k);
        end
    end
end
...
% [MODIFIED] Updated Y-axis label to reflect the 10uA max math
ylabel('Spike Count (Normalized to 10 \muA Max)', 'FontSize', 10);
...
fprintf('=================================================================\n');
fprintf('Directionality ISI figure generated.\n\n');