...
% 4. Universal Normalization Settings
...
for k = 1:nFiles
    ...
    % [MODIFIED] 3. Dynamic Normalization Engine (Independent Within-Set Logic)
    ...
    if ~isempty(idx_normAmp) && ~isempty(idx_normISI)
        % Loop through each experimental set independently
        for ss = 1:nSets_this
            % Extract the baseline explicitly and uniquely for the current set (ss)
            ...
            if ~isnan(baseline_val)
                if strcmpi(norm_method, 'subtract')
                    % Subtract baseline from THIS set only
                    Pop_Data(:, :, ss, k) = animal_means(:, :, ss) - baseline_val;
                    
                elseif strcmpi(norm_method, 'divide')
                    if baseline_val > 0
                        % Divide THIS set only by its own baseline
                        Pop_Data(:, :, ss, k) = animal_means(:, :, ss) / baseline_val;
                    else
                        fprintf('    -> Warning: Baseline for Set %d is 0. Cannot perform division.\n', ss);
                    end
                ...
                fprintf('    -> Warning: No valid %.1fuA at %.1fms baseline found for Set %d. Excluded from average.\n', norm_target_Amp, norm_target_ISI, ss);
            end
        end
    else
...
% Dynamic Y-axis label based on user settings
if strcmpi(norm_method, 'subtract')
