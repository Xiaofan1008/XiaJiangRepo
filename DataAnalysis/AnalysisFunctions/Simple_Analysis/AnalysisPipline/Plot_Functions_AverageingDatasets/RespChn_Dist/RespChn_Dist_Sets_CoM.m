...
%          (Distance where Occupancy drops below noise threshold)
% ============================================================
clear;
...
% Plot Settings
save_figures = false;
save_dir     = '/Volumes/MACData/Data/Data_Xia/Analyzed_Results/Group_Analysis/Spatial_Group_New/';
...
% Pooled.Amp_X.Sim_Probs   = [N_Sets x N_Bins]  (Density)
% Pooled.Amp_X.Sim_Global  = [N_Sets x N_Bins]  (Occupancy)
Pooled = struct();
All_Amps = [];
...
for f = 1:length(file_paths)
    ...
    for s = 1:length(SD.Set)
        ...
        for a = 1:length(SD.Set(s).Amp)
            ...
            % --- Calc Metrics Per Set ---
            % 1. Spatial Density (Prob = Count / Total Channels in Bin)
            ...
            % 2. Global Occupancy (Global = Count / Total Channels in Array)
            % This metric uses a Constant Denominator (Array Size)
...
for i = 1:length(All_Amps)
    ...
    % --- CALCULATE ABSOLUTE RANGE (R_ext) ---
    % Find max distance where Global Occupancy > 2% (0.02)
    threshold = 0.004; 
    ...
    % === FIG 1: SPATIAL DENSITY (Prob within Bin) ===
    ...
    xlabel('Distance (\mum)'); ylabel('Response Probability (Density)');
    ...
    % === FIG 2: GLOBAL OCCUPANCY (Count / Total Array) ===
    % This shows Potency + Spread unbiased by total count
    ...
    x_offset = b(1).XEndPoints - b(1).XData; 
    errorbar(bin_centers + x_offset(1), Avg_G_Sim, SEM_G_Sim, 'k.', 'LineWidth', 1, 'HandleVisibility','off');
    errorbar(bin_centers + x_offset(2), Avg_G_Seq, SEM_G_Seq, 'k.', 'LineWidth', 1, 'HandleVisibility','off');
...
% Plot Lines
plot(All_Amps, Sim_Rext, '-o', 'Color', 'b', 'LineWidth', 2, 'MarkerFaceColor','b', 'DisplayName','Simultaneous');
plot(All_Amps, Seq_Rext, '-s', 'Color', 'r', 'LineWidth', 2, 'MarkerFaceColor','r', 'DisplayName','Sequential');
...
function [metric_vec] = calc_metrics(dist, resp, valid, edges, mode)
    % Calculates Histograms based on mode
    ...
    % 2. Normalize based on Mode
    if strcmp(mode, 'prob')
        % Mode A: Spatial Density
        % Denom = Total Valid Channels IN THAT BIN
        for b = 1:length(edges)-1
            mask = dist >= edges(b) & dist < edges(b+1);
            mask_valid = mask & valid;
            n_valid = sum(mask_valid);
            if n_valid > 0
                metric_vec(b) = counts(b) / n_valid;
            else
                metric_vec(b) = NaN;
            end
        end
    ...
    elseif strcmp(mode, 'global')
        % Mode B: Global Occupancy
        % Denom = Total Valid Channels IN ENTIRE ARRAY
        % This measures global impact strength at that distance
        n_total_valid = sum(valid);
        if n_total_valid > 0
            metric_vec = counts / n_total_valid;
        else
            metric_vec = zeros(size(counts));
        end
    end
end