...
%% ---- Pulse Train Period (PTD) ----
% Extract PTD values (Delay between pulses)
PTD_all_us = cell2mat(StimParams(2:end, 6)); 
trialPTD_us = PTD_all_us(2:simultaneous_stim:end); % Usually stored on 2nd pulse row
...
if ~isempty(target_PTDs)
    ...
    if isempty(unique_PTDs)
        warning('No requested PTDs found in dataset. Plotting will be empty.');
    end
end
fprintf('Plotting Waveforms for PTDs: %s ms\n', num2str(unique_PTDs'));
...
if isempty(example_ch), error('All channels are empty in sp_use.'); end
...
for ich = spike_chn_start:spike_chn_end
    ...
    for s = 1:nSets
        % Loop through each PTD value (Filtered)
        for pi = 1:length(unique_PTDs)
            ...
            if isempty(all_waves), close(gcf); continue; end
            ...
            for b = 1:nBins
                ...
                % [Optional] Mark PTD line if it falls in this bin
                if ptd_val > 0
                    if ptd_val >= (b-1)*bin_ms && ptd_val < b*bin_ms
                        xline(ptd_val - (b-1)*bin_ms, 'k--');
                    end
                end
            end
