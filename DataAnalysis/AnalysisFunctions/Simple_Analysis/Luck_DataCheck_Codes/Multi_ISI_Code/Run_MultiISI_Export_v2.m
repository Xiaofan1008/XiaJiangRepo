...
%   - Exports the manually corrected responding-channel union.
%   - Also records separate response-population membership for A->B and
%     B->A, allowing each order to be averaged over its own channels.
...
single_folder = '/Volumes/MACData/Data/Data_Xia/DX024/Xia_ISI_10uA_Single1';

% Combined multi-ISI dataset containing simultaneous (PTD=0) and
% sequential (PTD>0) stimulation.
multiisi_folder = '/Volumes/MACData/Data/Data_Xia/DX024/Xia_ISI_10uA_SimSeq1';
...
for ipair = 1:numel(pair_defs)
    ...
    % Build a separate manually corrected response population for each
    % sequential order. Each mask is a union across the amplitudes and PTDs
    % selected for export, matching the population logic of the earlier
    % multi-ISI analysis. Bad channels are removed for that order only.
    [response_A_to_B,response_B_to_A] = order_response_masks( ...
        Multi,A,B,selected_positive_ptds,Amplitudes_To_Export, ...
        amp_tol,ptd_tol,nDepth);
    response_union = response_A_to_B | response_B_to_A;
    ...
    ChannelTable = build_channel_table(selected_depth_channels,Single,Multi, ...
        Electrode_Type,stimA,stimB,response_A_to_B,response_B_to_A);

    fprintf('Responding channels exported: %d\n',height(ChannelTable));
    fprintf('  A->B response population: %d channels\n', ...
        sum(ChannelTable.Responsive_A_to_B));
    fprintf('  B->A response population: %d channels\n', ...
        sum(ChannelTable.Responsive_B_to_A));
    ...
    for id = 1:numel(descriptors)
        ...
        for ia = 1:numel(amps)
            ...
            for jc = 1:height(ChannelTable)
                ...
                % Bad-channel labels belong to stimulation sets. When a
                % channel is bad for one order but usable for the other,
                % remove only trials belonging to the bad set.
                clean_trials = remove_bad_channel_trials(clean_trials, ...
                    Dataset.bad_channels,Dataset.experiment.set_index,depth_ch);
    ...
    ModelData.metadata = struct( ...
        ...
        'response_population_rule', ...
            'separate manually corrected union for each sequential order');
    ...
    %     dataset_id,pair_tag);
    output_name = sprintf('%s_MultiISI_ModelData.mat', ...
        dataset_id);
...
function clean_trials = remove_bad_channel_trials(trials,B,set_index,depth_channel)
% Remove only trials whose stimulation set marks this recording channel bad.
clean_trials = trials(:).';
if isempty(clean_trials) || ~B.file_found, return; end
keep = true(size(clean_trials));
for k = 1:numel(clean_trials)
    tr = clean_trials(k);
    si = set_index(tr);
    if si >= 1 && si <= numel(B.per_set) && ...
            ismember(depth_channel,B.per_set{si})
        keep(k) = false;
    end
end
...
function [mask_A_to_B,mask_B_to_A] = order_response_masks(D,A,B, ...
        positive_ptds,requested_amps,amp_tol,ptd_tol,nDepth)
% Construct manually corrected response unions independently for each order.
mask_A_to_B = one_order_response_mask(D,A,B,positive_ptds, ...
    requested_amps,amp_tol,ptd_tol,nDepth);
mask_B_to_A = one_order_response_mask(D,B,A,positive_ptds, ...
    requested_amps,amp_tol,ptd_tol,nDepth);
end

function mask = one_order_response_mask(D,first_electrode,second_electrode, ...
        positive_ptds,requested_amps,amp_tol,ptd_tol,nDepth)
E = D.experiment;
mask = false(1,nDepth);

if size(E.stim_names_per_trial,2) < 2, return; end
order_trial_mask = strcmp(E.stim_names_per_trial(:,1),first_electrode) & ...
    strcmp(E.stim_names_per_trial(:,2),second_electrode);
if ~any(order_trial_mask), return; end

selected_ptds = [0 double(positive_ptds(:).')];
ptd_selected = false(size(E.trial_ptd_ms));
for ptd = selected_ptds
    ptd_selected = ptd_selected | abs(E.trial_ptd_ms-ptd)<=ptd_tol;
end
eligible_trials = find(order_trial_mask & ptd_selected);
if isempty(eligible_trials), return; end

amps = selected_amplitudes(E.trial_amplitude(eligible_trials), ...
    requested_amps,amp_tol);
order_sets = unique(E.set_index(eligible_trials)).';
for amp = amps
    for ptd = selected_ptds
        trials = find(order_trial_mask & ...
            abs(E.trial_amplitude-amp)<=amp_tol & ...
            abs(E.trial_ptd_ms-ptd)<=ptd_tol);
        if isempty(trials), continue; end
        set_indices = unique(E.set_index(trials)).';
        mask = mask | responding_mask(D,set_indices,amp,ptd, ...
            amp_tol,ptd_tol,nDepth);
    end
end

% Bad-channel exclusions remain order-specific rather than being combined
% across both stimulation orders.
bad = bad_channels_for_sets(D.bad_channels,order_sets);
bad = bad(bad>=1 & bad<=nDepth);
mask(bad) = false;
end
...
function T = build_channel_table(depth_channels,Single,Multi,type,stimA,stimB, ...
        response_A_to_B,response_B_to_A)
...
Responsive_A_to_B = logical(response_A_to_B(depth_channels).');
Responsive_B_to_A = logical(response_B_to_A(depth_channels).');
...
T = table(ChannelIndex,DepthChannel,SpCellIndex_Single,SpCellIndex_MultiISI, ...
    Responsive_A_to_B,Responsive_B_to_A,Shank,LocalSite,X_um,Y_um, ...
