...
% Figures to display:
%   1 = dataset/trial-count overview
%   2 = raster + PSTH (one figure per recording channel)
%   3 = per-trial spike-count distributions (one figure per channel)
%   4 = spike count/trial versus amplitude (four channels per figure)
%   5 = average amplitude curves across all responding channels
Plot_Figures = [1 2 3 4 5];

% Channel selection:
%   'all'       = every channel saved by Stage C1
%   'responding'= saved channels marked as responding-union channels
%   numeric     = explicit DEPTH-channel list, for example [1 3 5 8]
Channels_To_Plot = 'responding';

% Empty means all amplitudes saved by Stage C1.
Amplitudes_To_Plot = [];

% All windows must lie inside the raster window saved by Stage C1.
raster_win_ms   = [-50 80];
baseline_win_ms = [-50 -5];
count_win_ms    = [2 20];

% PSTH settings. Smoothing uses a centred moving average and needs no
% external toolbox.
psth_bin_ms    = 1;
psth_smooth_ms = 5;

% Figure 4 uses baseline-corrected counts so that the A+B linear sum does
% not count spontaneous baseline activity twice.
...
if size(C.Data,3) ~= 5
    error('StageC2:ConditionCount','C2 expects five stimulation conditions.');
end
...
condition_names = {'A alone','B alone','A+B simultaneous', ...
    'A->B (5 ms)','B->A (5 ms)'};
condition_short = {'A','B','A+B','A->B','B->A'};
condition_colors = [0.10 0.45 0.80; 0.85 0.33 0.10; 0.15 0.65 0.30; ...
                    0.60 0.25 0.70; 0.15 0.65 0.70];
...
if ismember(2,Plot_Figures)
    ...
    for ir = 1:numel(selected_rows)
        ...
        tl = tiledlayout(numel(selected_amp_indices),5, ...
        ...
        for iaLocal = 1:numel(selected_amp_indices)
            ...
            max_rate_this_amp = 0;
            cached_rates = cell(1,5);
            cached_spikes = cell(1,5);
            for ic = 1:5
                D = C.Data(row,ia,ic);
...
if ismember(3,Plot_Figures)
    for ir = 1:numel(selected_rows)
        ...
        yMax = 1;
        all_counts = cell(numel(selected_amp_indices),5);
        ...
        for iaLocal = 1:numel(selected_amp_indices)
            ...
            mA = recalculate_metrics(C.Data(row,ia,1).spike_times_ms, ...
                baseline_win_ms,count_win_ms);
            mB = recalculate_metrics(C.Data(row,ia,2).spike_times_ms, ...
                baseline_win_ms,count_win_ms);
        ...
        for iaLocal = 1:numel(selected_amp_indices)
            ...
            errorbar(ax,6,dot_linear_mean(iaLocal),dot_linear_sem(iaLocal), ...
                'Color',linear_color,'LineStyle','none','LineWidth',1.6, ...
                'CapSize',9);
            plot(ax,6,dot_linear_mean(iaLocal),'d','MarkerSize',7, ...
                'MarkerFaceColor','w','MarkerEdgeColor',linear_color, ...
                'LineWidth',1.4);
            xlim(ax,[0.5 6.5]); ylim(ax,[-0.2 yMax]);
            xticks(ax,1:6); xticklabels(ax,dot_labels);
            xtickangle(ax,25);
...
if ismember(5,Plot_Figures)
    ...
    channel_condition_means = nan(nPopulation,5,numel(selected_amp_indices));
    ...
    curve_handles(6) = errorbar(ax,selected_amps,population_linear_mean, ...
...
if ismember(4,Plot_Figures)
    ...
    for page = 1:nPages
        ...
        for q = 1:numel(page_rows)
            ...
            means = nan(5,numel(selected_amp_indices));
            sems = nan(5,numel(selected_amp_indices));
            ...
            curve_handles(6) = errorbar(ax,selected_amps,linear_mean,linear_sem, ...
