...
% Use 'all' for every condition present in the package, or examples such as:
...
if numel(D.conditions) ~= numel(D.condition_definitions) || ...
        numel(D.conditions) < 4
    error('StageD2:ConditionCount', ...
        'The package must contain matching condition data and definitions.');
end
...
condition_colors = colors_for_conditions({D.conditions.code});

% This version intentionally prints no routine information in the Command
% Window. MATLAB errors are still shown if the input or settings are invalid.
...
for icLocal = 1:numel(condition_indices)
    ...
    for iaLocal = 1:numel(amplitude_indices)
        ...
        title(tl,sprintf('%s | %s | %.6g uA | %s', ...
            D.pair_key,condition.label,selected_amplitudes(iaLocal),pulse_text), ...
...
function colors = colors_for_conditions(codes)
% Assign colors by condition code so four-condition exports retain the
% correct sequential-order color even when the other order is absent.
...
for k = 1:numel(codes)
    switch codes{k}
        ...
        otherwise
            error('StageD2:UnknownCondition', ...
                'Unknown condition code in ModelData: %s',codes{k});
    end
end
end
