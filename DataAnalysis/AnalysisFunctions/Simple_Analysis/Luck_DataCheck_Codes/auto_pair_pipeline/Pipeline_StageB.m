...
if isempty(config_file) || ~isfile(config_file)
    error('AutoPairQC:MissingConfig', ...
        'Run Pipeline_StageB through Run_AutoPairQC.m.');
end
tmp_cfg = load(config_file,'PipelineConfig');
PipelineConfig = tmp_cfg.PipelineConfig;
stage_a_file = PipelineConfig.stage_a_file;
...
for ip = 1:numel(pair_defs)
    ...
    for ia = 1:numel(all_amps)
        ...
        R.sim_parameter_consistent = simultaneous_order_parameters_agree( ...
            Esim, trAB_raw, A, B);
        has_core_conditions = all(R.condition_available(1:3));
        has_any_sequential_order = any(R.condition_available(4:5));
        if has_core_conditions && has_any_sequential_order && ...
                ~R.sim_parameter_consistent
            has_parameter_problem = true;
        end

        % A usable amplitude requires A alone, B alone, simultaneous A+B,
        % and at least one sequential order. The reverse order is useful
        % when present, but is not mandatory.
        R.is_complete = has_core_conditions && has_any_sequential_order && ...
            R.sim_parameter_consistent;
        if R.is_complete
            R.potential_balanced_count = min( ...
                R.clean_trial_count(R.condition_available));
        else
    ...
    P.available_condition_order = {'A','B','AB'};
    common_seq_orders = false(1,2);
    if any(complete_mask)
        complete_availability = vertcat(amp_results(complete_mask).condition_available);
        common_seq_orders = all(complete_availability(:,4:5),1);
        if common_seq_orders(1)
            P.available_condition_order{end+1} = 'A_to_B';
        end
        if common_seq_orders(2)
            P.available_condition_order{end+1} = 'B_to_A';
        end

        % Only response labels belonging to conditions that will actually
        % be stored in C1/D1 are used to select the responding population.
        for ia = find(complete_mask)
            R = amp_results(ia);
            included_resp = R.responding_depth_channels.AB;
            if common_seq_orders(1)
                included_resp = union(included_resp, ...
                    R.responding_depth_channels.A_to_B);
            end
            if common_seq_orders(2)
                included_resp = union(included_resp, ...
                    R.responding_depth_channels.B_to_A);
            end
            pair_resp_union(included_resp) = true;
        end
    end
    ...
    if has_parameter_problem
        P.status = 'QC_PROBLEM';
    elseif ~all(condition_exists_any(1:3)) || ~any(condition_exists_any(4:5))
        P.status = 'INCOMPLETE';
    elseif isempty(P.complete_amplitudes_uA)
        P.status = 'NO_COMMON_AMPLITUDE';
    elseif ~any(common_seq_orders)
        % This unusual case means different amplitudes contain different
        % orders, with no one sequential order shared by every amplitude.
        % Stopping is safer than silently exporting unlike conditions.
        P.status = 'INCONSISTENT_SEQ_ORDER_ACROSS_AMPLITUDES';
    ...
    elseif all(common_seq_orders)
        P.status = 'COMPLETE_BOTH_ORDERS';
    elseif common_seq_orders(1)
        P.status = 'COMPLETE_A_TO_B_ONLY';
    else
        P.status = 'COMPLETE_B_TO_A_ONLY';
    end

    P.is_exportable = startsWith(P.status,'COMPLETE_');
    P.is_structurally_complete = P.is_exportable || ...
...
ConditionsRetained = strings(numel(Pairs),1);
for ip = 1:numel(Pairs)
    CompleteAmplitudes_uA(ip) = string( ...
        vector_text(Pairs(ip).complete_amplitudes_uA));
    ConditionsRetained(ip) = string(strjoin( ...
        Pairs(ip).available_condition_order,', '));
end
...
PairSummary = table(PairIndex, PairKey, ElectrodeA, ElectrodeB, Status, ...
    CompleteAmplitudes_uA, ConditionsRetained, CommonGoodChannels, RespondingUnion, ...
...
StageB.n_complete_pairs = sum([Pairs.is_exportable]);
...
P = struct('pair_index',[], 'key','', 'electrode_A','', 'electrode_B','', ...
    ...
    'condition_type_exists',false(1,5), ...
    'available_condition_order',{{}}, 'status','', 'is_exportable',false, ...
...
for ip = 1:numel(StageB.pairs)
    ...
    dual_print(fid, 'Conditions retained: %s\n', ...
        strjoin(P.available_condition_order, ', '));
