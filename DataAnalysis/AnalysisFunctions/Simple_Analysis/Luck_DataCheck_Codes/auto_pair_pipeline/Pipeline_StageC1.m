...
if isempty(config_file) || ~isfile(config_file)
    error('AutoPairQC:MissingConfig', ...
        'Run Pipeline_StageC1 through Run_AutoPairQC.m.');
end
tmp_cfg = load(config_file,'PipelineConfig');
PipelineConfig = tmp_cfg.PipelineConfig;
stage_b_file = PipelineConfig.stage_b_file;
pair_index = PipelineConfig.current_pair_index;
channel_mode = PipelineConfig.channel_mode;
manual_depth_channels = PipelineConfig.manual_depth_channels;
amplitudes_to_check = PipelineConfig.amplitudes_to_check;
...
single_bad_trials = PipelineConfig.single_bad_trials;
single_review_complete = PipelineConfig.single_review_complete;
require_same_stim_shank = PipelineConfig.require_same_stim_shank;
expected_stim_spacing_um = PipelineConfig.expected_stim_spacing_um;
geometry_tolerance_um = PipelineConfig.geometry_tolerance_um;
...
if ~isfield(PairB,'is_exportable') || ~PairB.is_exportable
    error('StageC1:IncompletePair', ...
        'Pair %d (%s) is not eligible for QC/export.', pair_index, PairB.key);
end
...
%% ==================== EXTRACT & CALCULATE =============================
condition_order = PairB.available_condition_order;
if ~iscell(condition_order) || numel(condition_order) < 4 || ...
        ~isequal(condition_order(1:3),{'A','B','AB'}) || ...
        ~any(ismember(condition_order,{'A_to_B','B_to_A'}))
    error('StageC1:InvalidConditionDefinition', ...
        'Stage B did not provide a valid dynamic condition list.');
end
[condition_labels,condition_sources] = describe_conditions(condition_order, ...
    PairB.electrode_A,PairB.electrode_B,StageB.config.seq_PTD_ms);
...
for ia = 1:nAmp
    ...
    for icond = 1:nCondition
        condition_code = condition_order{icond};
        trial_ids = AmpB.clean_trial_ids.(condition_code);
        if strcmp(condition_code,'A') || strcmp(condition_code,'B')
            trial_ids = setdiff(trial_ids,single_bad_trials,'stable');
        end
        trial_ids = trial_ids(:).';
...
StageC1.condition_sources = condition_sources;
...
try
    ...
    if ~(meta.size(1) == 1 || meta.size(2) == 1)
        error('StageC1:UnexpectedSpCorrShape', ...
            'sp_corr must be a row or column cell array.');
    end
    % MatFile objects reject an irregular vector such as [32 8 1 ...].
    % Reading one requested cell at a time avoids loading the complete,
    % potentially multi-gigabyte sp_corr variable and preserves the exact
    % requested channel order.
    raw_cells = cell(1,numel(hardware_indices));
    for kLoad = 1:numel(hardware_indices)
        if meta.size(1) == 1
            one_cell = M.sp_corr(1,hardware_indices(kLoad));
        else
            one_cell = M.sp_corr(hardware_indices(kLoad),1);
        end
        raw_cells{kLoad} = one_cell{1};
    end
    partial_ok = true;
catch ME
    warning('StageC1:PartialLoadFailed', ...
        ['Per-channel matfile loading failed (%s). Stage C1 will load the ' ...
...
function [labels,sources] = describe_conditions(codes,A,B,seq_ptd)
labels = cell(size(codes));
sources = cell(size(codes));
for k = 1:numel(codes)
    switch codes{k}
        case 'A'
            labels{k} = sprintf('%s alone',A);
            sources{k} = 'single';
        case 'B'
            labels{k} = sprintf('%s alone',B);
            sources{k} = 'single';
        case 'AB'
            labels{k} = sprintf('%s + %s simultaneous',A,B);
            sources{k} = 'sim';
        case 'A_to_B'
            labels{k} = sprintf('%s -> %s (%.3g ms)',A,B,seq_ptd);
            sources{k} = 'seq';
        case 'B_to_A'
            labels{k} = sprintf('%s -> %s (%.3g ms)',B,A,seq_ptd);
            sources{k} = 'seq';
        otherwise
            error('StageC1:UnknownCondition','Unknown condition code %s.',codes{k});
    end
end
end
