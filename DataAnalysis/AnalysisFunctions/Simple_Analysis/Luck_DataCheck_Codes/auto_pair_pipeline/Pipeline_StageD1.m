...
if isempty(config_file) || ~isfile(config_file)
    error('AutoPairQC:MissingConfig', ...
        'Run Pipeline_StageD1 through Run_AutoPairQC.m.');
end
...
export_folder = PipelineConfig.model_export_folder;
output_filename = '';
balancing_random_seed = PipelineConfig.balancing_random_seed;
allow_overwrite = PipelineConfig.allow_modeldata_overwrite;
...
if size(C.Data,3) ~= nCondition || nCondition < 4 || ...
    ...
    error('StageD1:ConditionMismatch', ...
        'C1 must contain A, B, AB, and at least one sequential order.');
end
...
condition_labels = cell(1,nCondition);
condition_sources = cell(1,nCondition);
electrode_orders = cell(1,nCondition);
pulse_times = cell(1,nCondition);
for ic = 1:nCondition
    [condition_labels{ic},condition_sources{ic},electrode_orders{ic}, ...
        pulse_times{ic}] = describe_condition(condition_order{ic},C, ...
...
for ic = 1:nCondition
    ...
    ConditionDefinitions(ic).code = condition_order{ic};
...
for ic = 1:nCondition
    ...
    Conditions(ic).code = condition_order{ic};
...
if numel(M.conditions) < 4 || ...
        ~all(ismember({'A','B','AB'},{M.conditions.code})) || ...
        ~any(ismember({'A_to_B','B_to_A'},{M.conditions.code}))
    error('StageD1:PackageConditionCount', ...
        'Exported package lacks a required stimulation condition.');
end
...
function [label,source,electrodes,pulse_times] = ...
        describe_condition(code,C,seq_ptd)
switch code
    case 'A'
        label = 'A alone';
        source = 'single';
        electrodes = {C.electrode_A};
        pulse_times = 0;
    case 'B'
        label = 'B alone';
        source = 'single';
        electrodes = {C.electrode_B};
        pulse_times = 0;
    case 'AB'
        label = 'A+B simultaneous';
        source = 'sim';
        electrodes = {C.electrode_A,C.electrode_B};
        pulse_times = [0 0];
    case 'A_to_B'
        label = sprintf('A then B, %.3g ms',seq_ptd);
        source = 'seq';
        electrodes = {C.electrode_A,C.electrode_B};
        pulse_times = [0 seq_ptd];
    case 'B_to_A'
        label = sprintf('B then A, %.3g ms',seq_ptd);
        source = 'seq';
        electrodes = {C.electrode_B,C.electrode_A};
        pulse_times = [0 seq_ptd];
    otherwise
        error('StageD1:UnknownCondition','Unknown condition code %s.',code);
end
end
