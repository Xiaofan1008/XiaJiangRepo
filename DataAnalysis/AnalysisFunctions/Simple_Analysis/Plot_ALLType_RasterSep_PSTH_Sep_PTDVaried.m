%% ============================================================
%   SINGLE vs SEQUENTIAL Raster + PSTH (SEPARATE FIGURES)
%   With SINGLE split per stimulation set
% ============================================================
...
%% ===================== USER INPUTS ===================== %%
folder_single = '/Volumes/MACData/Data/Data_Xia/DX013/Xia_Exp1_Single4_251128_150137';
folder_seq    = '/Volumes/MACData/Data/Data_Xia/DX013/Xia_Exp1_Seq_Sim4_251128_150648';
...
cmap_ptd = lines(20);   % PTD colors
cmap_single = lines(10); % Single sets colours
...
% PTDs
ptd_us_all = cell2mat(Stim(2:end,6));
PTD_us = ptd_us_all(2:simN:end);
...
for s = 1:D_seq.nSets
    ...
    %% --------------------------------------------------------
    %            FIGURE 1 — ALL RASTERS (separate)
    %% --------------------------------------------------------
    nRaster = D_single.nSets + numel(PTD_vals);
    figR = figure('Color','w','Position',[150 80 850 900]);
    tlR = tiledlayout(figR, nRaster, 1, 'TileSpacing','compact','Padding','compact');
    title(tlR, sprintf('RASTER   Ch %d   %.0f µA   Set [%s]', ...
        target_channel, amp_val, strjoin(string(stimCh),'→')));

    %% ============== SINGLE RASTERS — SEPARATE PER SET ==============
    S_ch_single = D_single.sp{d(target_channel)};
    for ss = 1:D_single.nSets
        
        ax = nexttile(tlR); hold(ax,'on'); box(ax,'off');

        stimSet = D_single.uniqueComb(ss, D_single.uniqueComb(ss,:)>0);
        title(ax, sprintf('Single — Set [%s]', strjoin(string(stimSet),',')));

        trial_ids_single = find(D_single.combClass==ss & D_single.trialAmps==amp_val);

        y=0;
        for tr = trial_ids_single'
            t0 = D_single.trig(tr)/FS*1000;
            tt = S_ch_single(:,1);
            tt = tt(tt>=t0+ras_win(1) & tt<=t0+ras_win(2)) - t0;

            scatter(ax,tt,y*ones(size(tt)),12, cmap_single(ss,:), 'filled');
            y=y+1;
        end
    ...
    for p = 1:numel(PTD_vals)
        ...
        ax = nexttile(tlR); hold(ax,'on'); box(ax,'off');
        title(ax, sprintf('Seq — PTD %d µs', ptd));
    ...
    %% --------------------------------------------------------
    %            FIGURE 2 — PSTHs (Single + Seq)
    %% --------------------------------------------------------
    figP = figure('Color','w','Position',[1050 80 850 600]);
    tlP = tiledlayout(figP, 2, 1, 'TileSpacing','compact','Padding','compact');
    title(tlP, sprintf('PSTH   Ch %d   %.0f µA   Set [%s]', ...
        target_channel, amp_val, strjoin(string(stimCh),'→')));


    %% ============== SINGLE PSTH PER SET ==============
    ax = nexttile(tlP); hold(ax,'on'); box(ax,'off');
    title(ax,'Single PSTH — Per Set');

    for ss = 1:D_single.nSets

        trial_ids_single = find(D_single.combClass==ss & D_single.trialAmps==amp_val);
        if isempty(trial_ids_single), continue; end
        ...
        rate = filter(g,1, counts/(numel(trial_ids_single)*bin_s));

        stimSet = D_single.uniqueComb(ss, D_single.uniqueComb(ss,:)>0);
        plot(ax, ctrs, rate, 'LineWidth', 2, ...
            'Color', cmap_single(ss,:), ...
            'DisplayName', sprintf('Single [%s]', strjoin(string(stimSet),',')));
    end
    ...
    for p = 1:numel(PTD_vals)
        ...
        plot(ax, ctrs, rate, 'LineWidth', 2, ...
            'Color', cmap_ptd(p,:), ...
            'DisplayName', sprintf('PTD %d µs', ptd));
    end
