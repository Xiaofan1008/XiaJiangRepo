...
%% === User Input === %%
% Folder paths
folder_single = '/Volumes/MACData/Data/Data_Xia/DX011/Xia_Exp1_Single6_251106_162313';
folder_simul  = '/Volumes/MACData/Data/Data_Xia/DX011/Xia_Exp1_Sim6_251106_163406';
folder_seq    = '/Volumes/MACData/Data/Data_Xia/DX011/Xia_Exp1_Seq6_5ms_251106_164125';
...
%% === Helper anonymous function to zero-out negatives === %%
clip_neg = @(x) max(x, 0);  % NEW: replaces negative values with 0

%% === 1. Plot Single Stimulation === %%
for s = 1:numel(FR_single)
    amps = FR_single(s).amps(:);
    % mean_FR = FR_single(s).mean(ch_to_plot,:);
    mean_FR = clip_neg(FR_single(s).mean(ch_to_plot,:));  % NEW
    sem_FR  = FR_single(s).sem(ch_to_plot,:);
    stimCh  = FR_single(s).stimCh;
    if all(isnan(mean_FR)), continue; end

    label_text = sprintf('Separate Set %d: Ch %s', s, num2str(stimCh));
    errorbar(amps, mean_FR, sem_FR, 'o-', 'LineWidth', lw, ...
        'MarkerSize', 6, 'Color', cmap(s,:), 'DisplayName', label_text);
end
...
for s = 1:numel(FR_simul)
    ...
    mean_FR = clip_neg(FR_simul(s).mean(ch_to_plot,:));  % NEW
    sem_FR  = FR_simul(s).sem(ch_to_plot,:);
    stimCh  = FR_simul(s).stimCh;
    if all(isnan(mean_FR)), continue; end

    label_text = sprintf('Simultaneous: Ch %s', num2str(stimCh));
...
for s = 1:numel(FR_seq)
    ...
    mean_FR = clip_neg(FR_seq(s).mean(ch_to_plot,:));  % NEW
    sem_FR  = FR_seq(s).sem(ch_to_plot,:);
    stimCh  = FR_seq(s).stimCh;
    if all(isnan(mean_FR)), continue; end

    label_text = sprintf('Sequential: Ch %s', num2str(stimCh));
    errorbar(amps, mean_FR, sem_FR, 's-', 'LineWidth', lw, ...
        'MarkerSize', 6, 'Color', cmap(4,:), 'DisplayName', label_text);
end
...
title(sprintf('Channel %d | Firing Rate', ch_to_plot), 'FontWeight', 'bold');
