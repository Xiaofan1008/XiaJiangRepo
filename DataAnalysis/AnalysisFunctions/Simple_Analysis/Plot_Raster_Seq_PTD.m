...
% Extract POST-TRIGGER DELAY (column 6)
postTrig_all = cell2mat(StimParams(2:end, 6));
postTrig = postTrig_all(2:simultaneous_stim:end);  % take 1 per trial
[PostTriggerVals, ~, postTrigIdx] = unique(postTrig);
n_POSTTRIG = numel(PostTriggerVals);
 ...
 edges = ras_win(1):bin_ms_raster:ras_win(2);
    ...
    for ich = raster_chn_start:raster_chn_end
        ...
        for si = 1:nSets
            ...
            for pti = 1:n_POSTTRIG
                postTrig_val = PostTriggerVals(pti);
                trials_this_delay = find(postTrigIdx == pti & combClass_win == set_id);
                if isempty(trials_this_delay), continue; end
                total_spikes = 0;
                for pi = 1:n_PULSE
                    ...
                    figure('Color','w','Name',sprintf('Ch %d | Set %s | Delay %d µs', ich, setLabel, postTrig_val));
                    ...
                    for ai = 1:n_AMP
                        amp_val = Amps(ai);
                        color = cmap(ai,:);
                        amp_trials = find(ampIdx == ai & postTrigIdx == pti & combClass_win == set_id);
                    ...
                    title(ax1, sprintf('Raster — Ch %d | Set: %s | Delay: %d µs', ich, setLabel, postTrig_val), 'Interpreter','none');
                ...
                fprintf('Ch %2d | Set %s | Post-Trigger Delay: %5d µs — Total spikes: %d\n', ...
                    ich, setLabel, postTrig_val, total_spikes);
            end
        end
    end