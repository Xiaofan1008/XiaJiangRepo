...
if isfield(S,'sp'), sp=S.sp; else, error('No sp'); end

if isempty(dir('*.trig.dat')), cleanTrig_sabquick; end
trig = loadTrig(0);

%% load clipped spike file
...
% amplitude filter mask
ampMask = ismember(trialAmps, plot_amps);   % <---- amplitude selection added
...
for ich = spike_chn_start:spike_chn_end
    ...
    for delay_i = 1:n_DELAYS
        ...
        for s = 1:nSets
            ...
            % combine stim-set + PTD + amplitude mask
            trial_mask = (combClass_win==set_id) & ...
                         (delayIdx==delay_i) & ...
                         ampMask;                         % <---- amplitude filter added
            ...
            figName = sprintf('Ch%d | Set %s | PTD %d us | Amps %s', ...
                        ich, strjoin(string(stimIdx),','), delay_val, mat2str(plot_amps));
            ...
            for b = 1:nBins
                ...
                for a = 1:n_AMP
                    if ~ismember(Amps(a), plot_amps), continue; end   % <---- skip unwanted amps
                ...
                title(sprintf('%d–%d ms (%d spikes)',(b-1)*bin_ms,b*bin_ms,cnt));
                xlabel('Time (ms)');
                ylabel('µV');
                ylim(y_lim);
                axis square;
            ...
            legend(h_leg, arrayfun(@(a) sprintf('%g µA',a), Amps(keepA), 'UniformOutput',false), ...
