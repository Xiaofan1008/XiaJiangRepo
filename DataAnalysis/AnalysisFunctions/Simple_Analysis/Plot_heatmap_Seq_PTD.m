...
% Extract post-trigger delay (column 6 in µs)
postDelay_all = cell2mat(StimParams(2:end,6));  % full list
postDelay = postDelay_all(2:simultaneous_stim:end);  % 1 per trial
[uniqueDelays, ~, delayIdx] = unique(postDelay);  % delay groups
n_DELAYS = numel(uniqueDelays);

fprintf('\n===== Found %d unique post-trigger delays =====\n', n_DELAYS);
disp(uniqueDelays');
...
%% === Average FR per Channel per Amp per Stim Set (by delay) ===
FR_heatmap_delay = cell(n_DELAYS, 1); % one heatmap per delay
FR_heatmap_norm_delay = cell(n_DELAYS, 1); % normalized per delay

for di = 1:n_DELAYS
    delay_val = uniqueDelays(di);
    trial_delay = find(postDelay == delay_val);  % trials with this delay

    FR_heatmap = nan(nChn, n_AMP, nSets);

    for si = 1:nSets
        trial_set = find(combClass_win == si);
        trial_both = intersect(trial_set, trial_delay); % same stim set + delay group
        for ai = 1:n_AMP
            trial_amp = find(trialAmps == Amps(ai));
            trials_here = intersect(trial_both, trial_amp);
            if isempty(trials_here), continue; end
            FR_heatmap(:, ai, si) = mean(FR_corrected(:, trials_here), 2);
        end
    end
    FR_heatmap_delay{di} = FR_heatmap;

 %% Normalize within stim set × shank
    FR_heatmap_norm_seq = nan(size(FR_heatmap));
    ...
    FR_heatmap_norm_delay{di} = FR_heatmap_norm_seq;
end
...
for si = 1:nSets
    ...
    stimChs = uniqueComb(si, uniqueComb(si,:) > 0);
    stimLabel = strjoin(arrayfun(@(x) sprintf('Ch%d', x), stimChs, 'UniformOutput', false), '+');
    sgtitle(sprintf('Stim Set %d: %s (Sequential)', si, stimLabel), 'FontWeight', 'bold', 'Interpreter','none');
