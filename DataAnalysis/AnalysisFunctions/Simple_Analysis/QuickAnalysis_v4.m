...
window_ms = [-100,300]; % time window for data extract
window_samps = round(window_ms/1000 * FS);
total_sample = diff(window_samps) + 1;
trialLength = diff(window_samps)+1;

thr_cutoff = 3.5; % threshold cutoff 
ART_thr = 150; % Artifact Threshold
...
for ch_idx = 1:nChn
    ...
    % thr(ch_idx) = -3 * sigma; % Compute threshold as -4.5 × standard deviation    
    thr(ch_idx) = -thr_cutoff * sigma;
end
...
for tr = 1:nTrials
    ...
    for ch = 1:nChn       
        ...
        % raw_interp(blank_range) = interp1(keep_range, raw(keep_range), blank_range, 'linear', 'nearest');

        % Ensure blank_range stays within bounds
        blank_range = blank_range(blank_range >= 1 & blank_range <= length(raw));
        raw_interp(blank_range) = interp1(keep_range, raw(keep_range), blank_range, 'linear');
...
% ch_spike = 25;
Spike_window = 10; % time window for the spike subplot (ms)
% QuickAnalysis.plot_spikes_by_time_grid(ch_spike, Spikes, FS, preS, postS, Amps, ampIdx, nTrials, Spike_window, 20)
for i = 1:nChn
    ch_spike = i;
    QuickAnalysis.plot_spikes_by_time_grid_perSet(ch_spike, Spikes, FS, preS, postS, Amps, ampIdx, nTrials, Spike_window, 300, combClass_win, uniqueComb);
end

% ch_filter_plot = 27;  % change to your desired channel
...
% QuickAnalysis.plot_filtered_signal_by_amplitude(MUA_all, ampIdx, Amps, t_axis, ch_filter_plot, filtered_window);
    QuickAnalysis.plot_filtered_signal_per_amp_StimSet(MUA_all, ampIdx, Amps, t_axis, ch_filter_plot, filtered_window, combClass_win, uniqueComb);
end
...
QuickAnalysis.raster_psth_stacked_SingleChn_per_set_amp_same_color(17, Amps, ampIdx, Spikes, combClass_win, uniqueComb);
% ------ (3) Heatmap ------ %
