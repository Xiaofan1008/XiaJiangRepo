...
%     end
% end
% % save('max_FR_per_channel_Simultaneous.mat', 'max_FR_sim');
...
%         shank_max = max_FR_per_shank(si, shank_id);
% 
%         if shank_max > 0
%             FR_heatmap_norm(ch,:,si) = FR_heatmap(ch,:,si) / shank_max;
%         else
%             FR_heatmap_norm(ch,:,si) = 0;
%         end
%     end
...
for si = 1:nSets
    ...
    stimChs = uniqueComb(si, uniqueComb(si,:) > 0);
    stimLabel = strjoin(arrayfun(@(x) sprintf('Ch%d', x), stimChs, 'UniformOutput', false), '+');
    sgtitle(sprintf('Stim Set %d: %s (Simultaneous)', si, stimLabel), 'FontWeight', 'bold', 'Interpreter','none');
