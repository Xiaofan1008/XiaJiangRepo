...
%         if max_val > 0
%             if si <= 2 % for single stim
%             % if si == 1 % for simultaneous / Sequential stim
...
%     end
% end
% % save('max_FR_per_channel_Single.mat', 'max_FR_single');
...
% Compute max firing rate per shank and stim set
max_FR_per_shank = zeros(nSets, 4);
for si = 1:nSets
    for sh = 1:4
        ch_idx = shank_map{sh};
...
% save('max_FR_pre_shank_single.mat', 'max_FR_per_shank_single_fix');
%% === Plot Normalized FR Heatmaps (Single Stim Sets Only) ===
...
for si = 1:nSets
    ...
    stimChs = uniqueComb(si, uniqueComb(si,:) > 0);
    stimLabel = strjoin(arrayfun(@(x) sprintf('Ch%d', x), stimChs, 'UniformOutput', false), '+');
    sgtitle(sprintf('Stim Set %d: %s (Separate)', si, stimLabel), 'FontWeight','bold', 'Interpreter','none');
    ...
    % Shared colorbar for figure
    cb = colorbar('Position', [0.93 0.11 0.02 0.78]);
    cb.Label.String = 'Normalized FR';
end
