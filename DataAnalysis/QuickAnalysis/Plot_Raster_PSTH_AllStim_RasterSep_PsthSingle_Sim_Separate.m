...
for amp_val = plot_amps
    ...
    for s = 1:size(D_single.uniqueComb,1)
        ...
        labelStr = sprintf('Single Set %d — Ch%s', s, num2str(stimCh));
        ...
        for tr = trial_ids'
            ...
            for k=1:numel(tt)
                plot(ax,[tt(k) tt(k)],[y y+0.8], ...
                     'Color', pastel_colors(s,:), 'LineWidth',1.1);
            end
            y=y+1;
        end
    ...
    %% PSTH for Single sets
    ax = nexttile(tl); box(ax,'off'); hold(ax,'on');
    ...
    for s = 1:size(D_single.uniqueComb,1)
        ...
        % rate = counts / max(1,(numel(trial_ids)*bin_s));
        ...
        plot(ax,ctrs,rate_s,'Color',pastel_colors(s,:), ...
             'LineWidth',2, 'DisplayName',sprintf('Single %d',s));
    end    
    xline(ax,0,'r--','HandleVisibility','off');  % remove from legend
    xlim(ax,ras_win);
    ylabel(ax,'Rate (sp/s)');
    legend(ax,'Box','off','Location','northeast');
    %% PSTH for Simultaneous + Sequential
    ax = nexttile(tl); box(ax,'off'); hold(ax,'on');
    ...
    rate_sim = filter(g,1, counts/(max(1,numel(trial_ids))*bin_s));
    plot(ax,ctrs,rate_sim,'Color',col_sim,'LineWidth',2,'DisplayName','Sim');
    ...
    rate_seq = filter(g,1, counts/(max(1,numel(trial_ids))*bin_s));
    plot(ax,ctrs,rate_seq,'Color',col_seq,'LineWidth',2,'DisplayName','Seq');
    
    xline(ax,0,'r--','HandleVisibility','off');
    xlim(ax,ras_win); ylabel(ax,'Rate (sp/s)'); xlabel(ax,'Time (ms)');
    legend(ax,'Box','off','Location','northeast');
