...
function raster_psth_stacked_SingleChn_per_set_amp_same_color(channels_to_plot, Amps, ampIdx, ...
    Spikes, combClass_win, uniqueComb)
% =============================================
% Raster + PSTH plots stacked by amplitude for selected channels
% One figure per channel per stim set
%
% Inputs:
%   - channels_to_plot: vector of channel indices to plot (e.g., [5 10 17])
%   - Amps:         [n_AMP x 1] list of unique amplitudes (µA)
%   - ampIdx:       [nTrials x 1] index (1-based) for amplitude per trial
%   - Spikes:       {nChn x nTrials} spike structure array with field .t_ms
%   - combClass_win:[nTrials x 1] stimulation set class ID
%   - uniqueComb:   [nSets x maxN] electrode index per stim set
% =============================================
    ...
    nAmps = numel(unique_amps);
    cmap = lines(nAmps);
    amp_to_idx = containers.Map(num2cell(unique_amps), num2cell(1:nAmps));
    ...
    for si = 1:nSets
        ...
        for ch = channels_to_plot(:)'  % <- user-defined channels only
            figure('Color','w','Name',sprintf('Ch %d | Set %s', ch, setLabel), 'NumberTitle','off');
            tl = tiledlayout(4,1,'TileSpacing','compact','Padding','compact');
            ax1 = nexttile([3 1]); hold(ax1,'on'); box(ax1,'off');
            ax2 = nexttile; hold(ax2,'on'); box(ax2,'off');
...
% One figure per stimulation set; colored by amplitude within each.
    ...
    yMax = 250; % y-axis limit for waveforms
