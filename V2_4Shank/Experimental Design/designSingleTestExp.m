...
function designSingleTestExp(DIR,AMP,DUR,IPD,E_DIAM,E_MAT,n_REP,CHN,PARAM,PULSE)
...
while (HAPPY)
    ...
    if ~(CASE)
        ...
        %% Post Trigger Delay
        setPTD = input('Do you want to set Post Trigger Delay? YES=1 NO=0 \n');    
        if setPTD
        ...
        PULSE = 500; %input('Please enter delay after stimulation in ms like so: [x y z]\n');
        %PULSE = 180;%150
    end
    AMP(AMP==0)=-1;
